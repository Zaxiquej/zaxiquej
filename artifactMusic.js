document.addEventListener("DOMContentLoaded", () => {
  const audioContext = new (window.AudioContext || window.webkitAudioContext)();
    ...
    const createKeyButton = (keyName, imageName, soundIndex) => {
        ...
        keyButton.dataset.soundIndex = soundIndex; // Store sound index
    ...
    const keyboardContainer = document.querySelector(".keyboard"); // Add this line

    keyMappings.forEach((mapping) => {
        const button = document.createElement("button");
        button.textContent = mapping.keyName;

        // Set background image
        const backgroundImageUrl = `url(image/creature${mapping.soundIndex}.png)`;
        button.style.backgroundImage = backgroundImageUrl;
