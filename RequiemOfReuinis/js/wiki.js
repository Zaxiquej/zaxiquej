...
const st={lang:savedLang,route:'overview',query:'',statusQuery:'',filters:{},sort:'idAsc',page:1,pageSize:30,results:[],enemyModifiers:{strong:false,thorns:false},saveCheck:null};
const UI_TEXT={zh:{pagination:'分页',previousPage:'上一页',nextPage:'下一页',beforeUpgrade:'升级前',afterUpgrade:'升级后'},en:{pagination:'Pagination',previousPage:'Previous',nextPage:'Next',beforeUpgrade:'Before Upgrade',afterUpgrade:'After Upgrade'},ja:{pagination:'ページ切り替え',previousPage:'前へ',nextPage:'次へ',beforeUpgrade:'強化前',afterUpgrade:'強化後'}};
...
function renderPagination(pages){const nav=$('#pagination');if(!nav)return;nav.setAttribute('aria-label',ui('pagination'));if(st.results.length<=st.pageSize){nav.innerHTML='';nav.hidden=true;return}nav.hidden=false;const numbered=paginationItems(st.page,pages).map(n=>n?'<button type="button" class="page-button'+(n===st.page?' active':'')+'" data-page="'+n+'"'+(n===st.page?' aria-current="page"':'')+'>'+n+'</button>':'<span class="page-ellipsis" aria-hidden="true">…</span>').join('');nav.innerHTML='<button type="button" class="page-button page-nav" data-page="'+(st.page-1)+'"'+(st.page===1?' disabled':'')+'>'+esc(ui('previousPage'))+'</button>'+numbered+'<button type="button" class="page-button page-nav" data-page="'+(st.page+1)+'"'+(st.page===pages?' disabled':'')+'>'+esc(ui('nextPage'))+'</button>'}
...
function researchDetail(x,level){const finite=x.maxLevel!=='\u221e'&&Number.isFinite(Number(x.maxLevel)),max=finite?Math.max(1,Number(x.maxLevel)):Infinity;level=Math.max(1,Math.floor(Number(level)||1));if(finite)level=Math.min(level,max);const picker=finite?'<select data-research-level="'+x.id+'">'+Array.from({length:max},(_,i)=>'<option value="'+(i+1)+'"'+(i+1===level?' selected':'')+'>'+esc(tr('level'))+' '+(i+1)+'</option>').join('')+'</select>':'<input type="number" min="1" step="1" value="'+level+'" data-research-level="'+x.id+'">';return '<header class="detail-header research-detail-header">'+ico(x.icon,'detail-icon research-detail-icon',64)+'<div><p class="detail-eyebrow">'+esc(tr('research'))+'</p><h2>'+esc(loc(x.name))+'</h2><div class="chip-row">'+chip(typeName(x.type),'blue')+chip(tr('maxLevel')+' '+x.maxLevel,'gold')+'</div></div></header><div class="level-picker"><label>'+esc(tr('level'))+'</label>'+picker+'</div>'+researchLevelPanel(x,level)+detailBlock(tr('unlock'),researchUnlockHtml(x))+detailBlock(tr('reward'),researchRewards(x))+detailBlock(tr('relatedMaterials'),relatedMaterialsHtml(x))}
...
document.addEventListener('click',e=>{const lang=e.target.closest('[data-lang]');if(lang){st.lang=lang.dataset.lang;try{localStorage.setItem('rorWikiLang',st.lang)}catch(err){}applyLanguage();return}const effect=e.target.closest('[data-derived-effect]');if(effect){const card=effect.closest('.derived-card'),mode=effect.dataset.derivedEffect;card.querySelectorAll('[data-derived-effect]').forEach(b=>{const active=b.dataset.derivedEffect===mode;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active))});card.querySelectorAll('.derived-effect').forEach(p=>p.hidden=!p.classList.contains(mode));return}const page=e.target.closest('[data-page]');if(page&&!page.disabled){st.page=Number(page.dataset.page)||1;renderSection();$('#sectionTitle').scrollIntoView({block:'start'});return}const tier=e.target.closest('[data-skill-tier]');if(tier){openDetail('skills',tier.dataset.skillId,tier.dataset.skillTier);return}const link=e.target.closest('[data-link-kind]');if(link){openDetail(link.dataset.linkKind,link.dataset.linkId);return}const route=e.target.closest('[data-route]');if(route){setRoute(route.dataset.route);return}const entry=e.target.closest('.entry-card');if(entry){openDetail(entry.dataset.kind,entry.dataset.id);return}const clear=e.target.closest('[data-clear]');if(clear){st.filters={};st.query='';st.page=1;$('#globalSearch').value='';renderSection();return}if(e.target===$('#mobileFilterButton')){$('#filters').classList.toggle('mobile-open');return}});
document.addEventListener('change',e=>{if(e.target===$('#saveFileInput')){inspectSaveFile(e.target.files&&e.target.files[0]);e.target.value='';return}if(e.target.matches('[data-enemy-modifier]')){st.enemyModifiers[e.target.dataset.enemyModifier]=e.target.checked;openDetail('enemies',e.target.dataset.enemyId);return}if(e.target.matches('[data-filter]')){st.filters[e.target.dataset.filter]=e.target.value;st.page=1;renderSection();return}if(e.target===$('#sortSelect')){st.sort=e.target.value;st.page=1;renderSection();return}if(e.target.matches('[data-research-level]'))openDetail('research',e.target.dataset.researchLevel,e.target.value)});
document.addEventListener('input',e=>{if(e.target===$('#statusSearch')){st.statusQuery=e.target.value;renderStatusGrid();return}if(e.target===$('#globalSearch')){st.query=e.target.value;st.page=1;if(st.route!=='overview'&&st.route!=='statuses')renderSection();return}if(e.target.matches('input[data-research-level]')){const id=e.target.dataset.researchLevel,value=e.target.value;if(value&&Number(value)>0)openDetail('research',id,value)}});
