...
Object.assign(U.zh,{bosses:'Boss',bossesIntro:'实际登场 Boss 的图像、初始属性、具体能力与出现位置。',bossLocation:'具体位置',coordinates:'坐标'});
Object.assign(U.en,{bosses:'Bosses',bossesIntro:'Sprites, initial stats, abilities, and exact locations for Bosses that appear in the game.',bossLocation:'Exact location',coordinates:'Coordinates'});
Object.assign(U.ja,{bosses:'ボス',bossesIntro:'実際に登場するボスの画像、初期能力、スキル、出現位置。',bossLocation:'出現位置',coordinates:'座標'});
const SECTIONS=['equipment','skills','research','enemies'];
const ROUTES=[...SECTIONS,'bosses','statuses','rankings'];
...
const st={lang:savedLang,route:'overview',query:'',statusQuery:'',filters:{},sort:'idAsc',page:1,pageSize:30,results:[],enemyModifiers:{strong:false,thorns:false},saveCheck:null,architectMode:false,researchDiscount:0,researchTree:false,treeTransform:{x:0,y:0,scale:1},treeInitialized:false,rankingType:'equipment'};
...
const rubyBaseText=v=>String(v==null?'':v).replace(/<ruby=([^>]+)>([^<]*)<\/ruby>/gi,'$2');
const rubyHtml=v=>{const s=String(v==null?'':v),re=/<ruby=([^>]+)>([^<]*)<\/ruby>/gi;let out='',last=0,m;while((m=re.exec(s))){out+=esc(s.slice(last,m.index))+'<ruby class="wiki-ruby">'+esc(m[2])+'<rt>'+esc(m[1])+'</rt></ruby>';last=re.lastIndex}return out+esc(s.slice(last))};
const plain=v=>String(v||'').replace(/<ruby=([^>]+)>([^<]*)<\/ruby>/gi,'$2 $1').replace(/\\n/g,' ').replace(/\r?\n/g,' ').replace(/\\[A-Za-z]+(?:\[[^\]]*\])?/g,' ').replace(/#\{[^}]+\}/g,'').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();
...
const chip=(x,c='')=>x?'<span class="chip '+c+'">'+rubyHtml(x)+'</span>':'';
...
function refIcon(kind,id){
 ...
 if(kind==='IB'){let n=D.refs.buffs[id];return '<span class="inline-buff">'+rubyHtml(loc(n)||tr('effect'))+'</span>';}
...
const sectionData=k=>k==='all'?SECTIONS.flatMap(q=>D[q].map(x=>Object.assign({_kind:q},x))):(D[k]||[]);function searchable(x,k){
 ...
 if(k==='equipment'){
  p.push(...(x.derived||[]).flatMap(d=>[loc(d.name),loc(d.description),loc(d.acquisition)]));
  if(x.tags&&Object.prototype.hasOwnProperty.call(x.tags,'ring'))p.push('戒指 ring 指輪');
  const fireCountdown=x.tags&&x.tags.skillCountDown?String(x.tags.skillCountDown).split(','):[];
  if((x.tags&&[502,534].includes(Number(x.tags.Askill)))||(x.tags&&(x.tags.curseF!==undefined||x.tags.pExplode!==undefined))||String(fireCountdown[2]||'').toLowerCase()==='fire')p.push('食火者 The Fire-Eater Fire Eater 火喰らい');
 }
 if(k==='skills'){
  p.push(loc(x.actor),loc(x.flavor),...x.variants.flatMap(v=>v.derived.flatMap(d=>[loc(d.name),loc(d.description),loc(d.acquisition)])));
  if([13,14,25,38,64,68,82,83,91,98,110,128,134].includes(x.id))p.push('食火者 The Fire-Eater Fire Eater 火喰らい');
 }
 ...
 if(k==='enemies'||k==='bosses')p.push(...x.maps.map(v=>loc(v.name)),...(x.bossLocations||[]).map(v=>loc(v.areaName)),...x.abilities.flatMap(a=>[loc(a.name),loc(a.description)]));
...
function filtered(k){
 ...
 let a=sectionData(k).filter(x=>{const z=x._kind||k;if(keywords.length&&!matchesSearchQuery(searchable(x,z),keywords))return false;
  ...
  if(z==='research'&&((f.type&&x.type!==f.type)||(selectedMaterials.length&&!(x.relatedItems||[]).some(c=>selectedMaterials.includes(String(c.itemId))))))return false;
  if((z==='enemies'||z==='bosses')&&((f.category&&!(x.categories||[x.category]).includes(f.category))||(f.map&&!(z==='bosses'?(x.bossLocations||[]).some(m=>String(m.areaId)===f.map):x.maps.some(m=>String(m.id)===f.map)))))return false;
  return true;
 });
 const kind=x=>x._kind||k,power=(x,z)=>(z==='enemies'||z==='bosses')?x.hp:z==='skills'?Math.max(...x.variants.map(v=>Number(v.tags.damage||v.tags.pdamage||v.tags.recover||0))):x.id,count=(x,z)=>z==='skills'?x.variants.reduce((n,v)=>n+v.derived.length,0):z==='research'?x.prerequisites.length:(z==='enemies'||z==='bosses')?x.abilities.length:0;
...
function renderOverview(){
 const q=D.summary,a=[['equipment',q.equipment,'⚔'],['skills',q.skills,'✦'],['research',q.research,'⚗'],['enemies',q.enemies,'♞'],['bosses',q.bosses||(D.bosses||[]).length,'♛'],['statuses',q.statuses||28,'◈']];
 ...
 ['Equipment','Skills','Research','Enemies','Bosses','Statuses'].forEach((x,i)=>$('#count'+x).textContent=a[i][1]);
...
function regularSaveArchive(fileName,raw){
 ...
 const actor=resolveSaveReference(actorData[1],data),learned=jsonExArray(actor&&actor._skills,data)||[],equipmentIds=[...equipmentIdSet],skillIds=[...new Set(learned.map(normalizeSkillId).filter(Boolean))];
...
function filterSelect(k,label,a,fmt=x=>x){
 ...
 return '<div class="filter-group"><label for="filter-'+k+'">'+esc(label)+'</label><select id="filter-'+k+'" data-filter="'+k+'"><option value="">'+esc(tr('all'))+'</option>'+a.map(x=>'<option value="'+esc(x)+'"'+(String(v)===String(x)?' selected':'')+'>'+esc(rubyBaseText(fmt(x)))+'</option>').join('')+'</select></div>';
}
function materialFilter(a){const materials=new Map(),selected=new Set(Array.isArray(st.filters.materials)?st.filters.materials:(st.filters.material?[st.filters.material]:[]));a.forEach(x=>(x.relatedItems||[]).forEach(c=>materials.set(String(c.itemId),c)));const rows=[...materials.values()].sort((x,y)=>Number(x.itemId)-Number(y.itemId));return '<div class="filter-group material-filter-group"><span>'+esc(tr('material'))+'</span><div class="material-filter-grid" role="group" aria-label="'+esc(tr('material'))+'">'+rows.map(c=>{const id=String(c.itemId),active=selected.has(id),name=loc(c.name);return '<button type="button" class="material-filter-button'+(active?' active':'')+'" data-material-filter="'+id+'" aria-pressed="'+String(active)+'" title="'+esc(name)+'">'+ico(c.icon,'material-filter-icon',34)+'<small>'+esc(name)+'</small></button>'}).join('')+'</div></div>'}
function renderFilters(k){
 const mobileClose='<button type="button" class="mobile-filter-close" data-close-mobile-filters aria-label="'+esc(mobileCloseText())+'">×</button>';
 if(k==='all'){$('#filters').innerHTML=mobileClose+'<div class="filter-title"><strong>'+esc(tr('searchResults'))+'</strong><button data-clear>'+esc(tr('clear'))+'</button></div><div class="filter-group"><span>'+esc(tr('keyword'))+'</span><p class="source-line">'+esc(st.query)+'</p></div>';return}
 const a=D[k];let h=mobileClose+'<div class="filter-title"><strong>'+esc(tr('filters'))+'</strong><button data-clear>'+esc(tr('clear'))+'</button></div>';
 if(k==='equipment')h+=filterSelect('slots',tr('slots'),values(a,x=>String(x.slots)).sort((x,y)=>Number(x)-Number(y)));
 if(k==='skills'){const actors=Object.fromEntries(D.actors.map(x=>[String(x.id),loc(x.name)]));h+=filterSelect('actor',tr('actor'),Object.keys(actors),x=>actors[x])}
 if(k==='research'){h+=filterSelect('type',tr('type'),values(a,x=>x.type),typeName);h+=materialFilter(a)}
 if(k==='enemies'||k==='bosses'){const m={};a.forEach(x=>(k==='bosses'?(x.bossLocations||[]).map(y=>({id:y.areaId,name:y.areaName})):x.maps).forEach(y=>m[y.id]=loc(y.name)));if(k==='enemies')h+=filterSelect('category',tr('category'),['boss','elite','common','summon'],x=>tr(x));h+=filterSelect('map',tr('appearance'),Object.keys(m).sort((x,y)=>Number(x)-Number(y)),x=>m[x])}
 $('#filters').innerHTML=h;
}
const sourceText=x=>x.sourceKnown?loc(x.acquisition):tr('none');
const enemyStatLabel=k=>st.lang==='ja'?({atk:'ATK',def:'DEF',gold:'G'}[k]||tr(k)):tr(k);
function card(x,k){
 ...
 if(k==='skills'){const v=x.variants[0];return '<article class="entry-card skill-entry-card" data-kind="'+k+'" data-id="'+x.id+'"><div class="card-head"><div class="skill-card-icon">'+ico(x.icon)+actorImg(x.actorImage,'actor-avatar-mini')+'</div><div><h2>'+rubyHtml(loc(x.name))+'</h2><small>'+rubyHtml(loc(x.actor))+'</small></div></div><div class="chip-row">'+chip('MP '+(Number(v.mpCost)||0),'blue')+chip(tr('uses')+' '+(Number(v.pp)||0),'gold')+'</div></article>'}
 ...
 const locations=k==='bosses'?(x.bossLocations||[]).map(point=>esc(loc(point.areaName))+' · '+esc(tr('coordinates'))+' '+point.x+', '+point.y).join('<br>'):x.maps.slice(0,3).map(m=>esc(loc(m.name))).join(' · ');
 return '<article class="entry-card" data-kind="'+k+'" data-id="'+x.id+'"><div class="card-head">'+portrait+'<div><h2>'+esc(loc(x.name))+'</h2><small>'+esc((x.categories||[x.category]).map(tr).join(' · '))+'</small></div></div><div class="statline"><span>'+enemyStatLabel('hp')+'<b>'+x.hp+'</b></span><span>'+enemyStatLabel('atk')+'<b>'+x.atk+'</b></span><span>'+enemyStatLabel('def')+'<b>'+x.def+'</b></span></div><div class="chip-row">'+x.abilities.slice(0,4).map(a=>chip(loc(a.name),a.key==='swift'?'gold':'')).join('')+'</div><div class="source-line">'+locations+'</div></article>';
}
function renderSection(){
 const k=st.route,tk=k==='all'?'searchResults':k,ik=k==='all'?'searchIntro':k+'Intro';
 $('#sectionEyebrow').textContent=k==='all'?'SEARCH':'DATABASE / '+k.toUpperCase();$('#sectionTitle').textContent=tr(tk);$('#sectionIntro').textContent=tr(ik);
 const treeActive=k==='research'&&st.researchTree,toggle=$('#researchTreeToggle');toggle.hidden=k!=='research';toggle.textContent=treeText(treeActive?'viewList':'viewTree');$('#catalogLayout').hidden=treeActive;$('#researchTreeView').hidden=!treeActive;
 if(treeActive){$('#resultCount').textContent=(D.research||[]).length;renderResearchTree();return}
 renderFilters(k);renderCatalogSaveChecker(k);
...
let researchTreeGraph=null,researchTreeResizeObserver=null,researchTreeDrag=null;
function buildResearchTreeGraph(){
 const research=D.research||[],byId=new Map(research.map(x=>[Number(x.id),x])),nodes=new Map(),edges=[],edgeSet=new Set(),parents=new Map(),outgoing=new Map();
 research.forEach(x=>nodes.set('r:'+x.id,{key:'r:'+x.id,kind:'research',id:Number(x.id),data:x,name:loc(x.name),icon:Number(x.icon)||0}));
 const addEdge=(from,to,kind)=>{const key=from+'>'+to+'>'+kind;if(edgeSet.has(key)||!nodes.has(from)||!nodes.has(to))return;edgeSet.add(key);edges.push({from,to,kind});if(!parents.has(to))parents.set(to,[]);parents.get(to).push(from);outgoing.set(from,(outgoing.get(from)||0)+1)};
 const materialNode=item=>{const key='material:'+item.id;if(!nodes.has(key))nodes.set(key,{key,kind:'material',id:Number(item.id),name:loc(item.name),icon:Number(item.icon)||0});return key};
 const mapNode=map=>{const key='map:'+map.id;if(!nodes.has(key))nodes.set(key,{key,kind:'map',id:Number(map.id),name:loc(map.name),icon:0});return key};
 research.forEach(x=>{const target='r:'+x.id;(x.prerequisites||[]).forEach(p=>{if(byId.has(Number(p.id)))addEdge('r:'+p.id,target,'research')});(x.unlockItems||[]).forEach(item=>addEdge(materialNode(item),target,'material'));(x.eventMaps||[]).forEach(map=>addEdge(mapNode(map),target,'map'))});
 const depthMemo=new Map(),depthOf=(id,visiting=new Set())=>{id=Number(id);if(depthMemo.has(id))return depthMemo.get(id);if(visiting.has(id))return 0;const x=byId.get(id);if(!x)return 0;const next=new Set(visiting);next.add(id);let depth=((x.unlockItems||[]).length||(x.eventMaps||[]).length)?1:0;(x.prerequisites||[]).forEach(p=>{if(byId.has(Number(p.id)))depth=Math.max(depth,depthOf(p.id,next)+1)});depthMemo.set(id,depth);return depth};
 research.forEach(x=>depthOf(x.id));
 const isolated=[],isolatedKeys=new Set(),layers=new Map(),nodeDepth=new Map();nodes.forEach(node=>{if(node.kind==='research'&&!parents.has(node.key)&&!outgoing.has(node.key)){isolated.push(node);isolatedKeys.add(node.key);return}const depth=node.kind==='research'?(depthMemo.get(node.id)||0):0;nodeDepth.set(node.key,depth);if(!layers.has(depth))layers.set(depth,[]);layers.get(depth).push(node)});
 const children=new Map(),adjacency=new Map();nodes.forEach((node,key)=>{if(!isolatedKeys.has(key))adjacency.set(key,[])});edges.forEach(edge=>{if(!children.has(edge.from))children.set(edge.from,[]);children.get(edge.from).push(edge.to);if(adjacency.has(edge.from)&&adjacency.has(edge.to)){adjacency.get(edge.from).push(edge.to);adjacency.get(edge.to).push(edge.from)}});
 const rawComponents=[],componentSeen=new Set();adjacency.forEach((_,start)=>{if(componentSeen.has(start))return;const keys=[],stack=[start];componentSeen.add(start);while(stack.length){const key=stack.pop();keys.push(key);(adjacency.get(key)||[]).forEach(next=>{if(!componentSeen.has(next)){componentSeen.add(next);stack.push(next)}})}rawComponents.push(keys)});
 const sourceCount=component=>component.reduce((n,key)=>n+(nodes.get(key).kind==='research'?0:1),0),stableComponentKey=component=>component.map(key=>{const node=nodes.get(key);return (node.kind==='material'?'0':node.kind==='map'?'1':'2')+String(node.id).padStart(6,'0')}).sort()[0]||'';
 rawComponents.sort((a,b)=>b.length-a.length||sourceCount(b)-sourceCount(a)||stableComponentKey(a).localeCompare(stableComponentKey(b)));const componentRank=new Map();rawComponents.forEach((component,rank)=>component.forEach(key=>componentRank.set(key,rank)));
 const kindRank={material:0,map:1,research:2},degree=node=>(parents.get(node.key)||[]).length+(children.get(node.key)||[]).length,componentOf=node=>componentRank.get(node.key)||0;
 layers.forEach(list=>list.sort((a,b)=>componentOf(a)-componentOf(b)||kindRank[a.kind]-kindRank[b.kind]||degree(b)-degree(a)||rubyBaseText(a.name).localeCompare(rubyBaseText(b.name),st.lang)||a.id-b.id));
 const layerDepths=[...layers.keys()].sort((a,b)=>a-b),normalizedPositions=()=>{const positions=new Map();layers.forEach(list=>list.forEach((node,index)=>positions.set(node.key,list.length>1?index/(list.length-1):.5)));return positions},reorderLayer=(depth,relations)=>{const list=layers.get(depth);if(!list||list.length<2)return;const previous=new Map(list.map((node,index)=>[node.key,index])),positions=normalizedPositions(),barycenter=node=>{const values=(relations.get(node.key)||[]).map(key=>positions.get(key)).filter(Number.isFinite);return values.length?values.reduce((sum,value)=>sum+value,0)/values.length:null};list.sort((a,b)=>{const component=componentOf(a)-componentOf(b);if(component)return component;const av=barycenter(a),bv=barycenter(b);if(av!==null&&bv!==null&&Math.abs(av-bv)>1e-6)return av-bv;if(av!==null&&bv===null)return -1;if(av===null&&bv!==null)return 1;return degree(b)-degree(a)||previous.get(a.key)-previous.get(b.key)})};
 for(let pass=0;pass<8;pass++){layerDepths.slice(1).forEach(depth=>reorderLayer(depth,parents));layerDepths.slice(0,-1).reverse().forEach(depth=>reorderLayer(depth,children))}
 const componentEdges=rawComponents.map(()=>[]);edges.forEach(edge=>{const rank=componentRank.get(edge.from);if(rank!==undefined&&rank===componentRank.get(edge.to))componentEdges[rank].push(edge)});
 const componentCrossings=rank=>{const localLayers=layerDepths.map(depth=>(layers.get(depth)||[]).filter(node=>componentOf(node)===rank)).filter(list=>list.length),columns=Math.max(1,...localLayers.map(list=>list.length)),points=new Map();localLayers.forEach((list,row)=>list.forEach((node,column)=>points.set(node.key,{x:(columns-list.length)/2+column,y:row})));const lines=componentEdges[rank],cross=(a,b,c)=>(b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x);let count=0;for(let i=0;i<lines.length;i++){const first=lines[i],a=points.get(first.from),b=points.get(first.to);for(let j=i+1;j<lines.length;j++){const second=lines[j];if(first.from===second.from||first.from===second.to||first.to===second.from||first.to===second.to)continue;const c=points.get(second.from),d=points.get(second.to),ab1=cross(a,b,c),ab2=cross(a,b,d),cd1=cross(c,d,a),cd2=cross(c,d,b);if(((ab1>0&&ab2<0)||(ab1<0&&ab2>0))&&((cd1>0&&cd2<0)||(cd1<0&&cd2>0)))count++}}return count};
 for(let pass=0;pass<6;pass++){let changed=false;rawComponents.forEach((_,rank)=>{let score=componentCrossings(rank);const depths=pass%2?layerDepths.slice().reverse():layerDepths;depths.forEach(depth=>{const layer=layers.get(depth),indices=[];layer.forEach((node,index)=>{if(componentOf(node)===rank)indices.push(index)});for(let i=0;i<indices.length-1;i++){const left=indices[i],right=indices[i+1],temp=layer[left];layer[left]=layer[right];layer[right]=temp;const next=componentCrossings(rank);if(next<score){score=next;changed=true}else{layer[right]=layer[left];layer[left]=temp}}})});if(!changed)break}
 const gapX=156,gapY=190,isolatedGapY=142,blockPadX=72,blockPadY=60,blockGap=150,stageMargin=90;
 const blocks=rawComponents.map((keys,rank)=>{const componentLayers=layerDepths.map(depth=>({depth,nodes:(layers.get(depth)||[]).filter(node=>componentOf(node)===rank)})).filter(layer=>layer.nodes.length),columns=Math.max(1,...componentLayers.map(layer=>layer.nodes.length));return {rank,kind:'component',layers:componentLayers,columns,width:blockPadX*2+columns*gapX,height:blockPadY*2+86+Math.max(0,componentLayers.length-1)*gapY}});
 if(isolated.length){isolated.sort((a,b)=>rubyBaseText(a.name).localeCompare(rubyBaseText(b.name),st.lang)||a.id-b.id);const columns=Math.max(1,Math.ceil(Math.sqrt(isolated.length*86/116))),rows=Math.ceil(isolated.length/columns);blocks.push({rank:rawComponents.length,kind:'isolated',nodes:isolated,columns,rows,width:blockPadX*2+columns*gapX,height:blockPadY*2+86+Math.max(0,rows-1)*isolatedGapY})}
 const orderedBlocks=blocks.slice().sort((a,b)=>b.height-a.height||b.width-a.width||a.rank-b.rank),totalArea=blocks.reduce((sum,block)=>sum+block.width*block.height,0),widest=Math.max(...blocks.map(block=>block.width));
 const packBlocks=limit=>{const shelves=[],placements=new Map();orderedBlocks.forEach(block=>{let shelf=null,bestSpace=Infinity;shelves.forEach(candidate=>{const nextWidth=candidate.used+(candidate.used?blockGap:0)+block.width;if(block.height<=candidate.height&&nextWidth<=limit){const space=limit-nextWidth;if(space<bestSpace){bestSpace=space;shelf=candidate}}});if(!shelf){shelf={y:shelves.reduce((sum,row)=>sum+row.height+(sum?blockGap:0),0),height:block.height,used:0,items:[]};shelves.push(shelf)}const x=shelf.used+(shelf.used?blockGap:0);shelf.items.push({block,x});shelf.used=x+block.width});let packedWidth=0;shelves.forEach(shelf=>{packedWidth=Math.max(packedWidth,shelf.used);shelf.items.forEach(item=>placements.set(item.block,{x:item.x,y:shelf.y+(shelf.height-item.block.height)/2}))});const packedHeight=shelves.length?shelves[shelves.length-1].y+shelves[shelves.length-1].height:0;return {placements,width:packedWidth,height:packedHeight}};
 const naturalWidth=Math.max(widest,Math.sqrt(totalArea)),candidates=[];for(let factor=.78;factor<=1.9;factor+=.06)candidates.push(packBlocks(Math.max(widest,naturalWidth*factor)));const packed=candidates.reduce((best,current)=>{const ratio=current.width/current.height,score=Math.abs(Math.log(ratio))*4+(current.width*current.height/totalArea)*.08,bestRatio=best.width/best.height,bestScore=Math.abs(Math.log(bestRatio))*4+(best.width*best.height/totalArea)*.08;return score<bestScore?current:best});
 blocks.forEach(block=>{const place=packed.placements.get(block),left=stageMargin+place.x,top=stageMargin+place.y;if(block.kind==='component'){block.layers.forEach((layer,row)=>{const offset=(block.columns-layer.nodes.length)*gapX/2;layer.nodes.forEach((node,column)=>{node.x=left+blockPadX+offset+(column+.5)*gapX;node.y=top+blockPadY+43+row*gapY})})}else{block.nodes.forEach((node,index)=>{const row=Math.floor(index/block.columns),column=index%block.columns,count=Math.min(block.columns,block.nodes.length-row*block.columns),offset=(block.columns-count)*gapX/2;node.x=left+blockPadX+offset+(column+.5)*gapX;node.y=top+blockPadY+43+row*isolatedGapY})}});
 const stageWidth=packed.width+stageMargin*2,stageHeight=packed.height+stageMargin*2;
 const outgoingEdges=new Map(),incomingEdges=new Map();edges.forEach(edge=>{if(!outgoingEdges.has(edge.from))outgoingEdges.set(edge.from,[]);if(!incomingEdges.has(edge.to))incomingEdges.set(edge.to,[]);outgoingEdges.get(edge.from).push(edge);incomingEdges.get(edge.to).push(edge)});
 const fanEdges=(groups,otherKey,offsetKey)=>groups.forEach(group=>{group.sort((a,b)=>nodes.get(a[otherKey]).x-nodes.get(b[otherKey]).x);const span=Math.min(50,Math.max(0,(group.length-1)*7));group.forEach((edge,index)=>edge[offsetKey]=group.length>1?-span/2+span*index/(group.length-1):0)});fanEdges(outgoingEdges,'to','fromOffset');fanEdges(incomingEdges,'from','toOffset');
 return {nodes,edges,width:stageWidth,height:Math.max(720,stageHeight)};
}
function researchTreeNodeHtml(node){
 const name=rubyHtml(node.name),title=esc(rubyBaseText(node.name));
 if(node.kind==='research'){const cap=node.data.maxLevel==='∞'?'∞':node.data.maxLevel;return '<button type="button" class="research-tree-node research" data-research-tree-id="'+node.id+'" style="left:'+(node.x-58)+'px;top:'+(node.y-43)+'px" title="'+title+'"><span class="tree-node-shape research">'+ico(node.icon,'research-tree-icon',40)+'<em>'+esc(treeText('maxLevel'))+' '+cap+'</em></span><strong>'+name+'</strong></button>'}
 if(node.kind==='material')return '<div class="research-tree-node source material" style="left:'+(node.x-58)+'px;top:'+(node.y-43)+'px" title="'+title+'"><span class="tree-node-shape material">'+ico(node.icon,'research-tree-icon',40)+'</span><strong>'+name+'</strong></div>';
 return '<div class="research-tree-node source map" style="left:'+(node.x-58)+'px;top:'+(node.y-43)+'px" title="'+title+'"><span class="tree-node-shape map"><i>⌖</i></span><strong>'+name+'</strong></div>';
}
function applyResearchTreeTransform(){const stage=$('#researchTreeStage');if(!stage)return;const t=st.treeTransform;stage.style.transform='translate3d('+t.x+'px,'+t.y+'px,0) scale('+t.scale+')';drawResearchTreeEdges()}
function drawResearchTreeEdges(){
 const graph=researchTreeGraph,viewport=$('#researchTreeViewport'),canvas=$('#researchTreeEdges');if(!graph||!viewport||!canvas||viewport.hidden)return;const w=viewport.clientWidth,h=viewport.clientHeight;if(!w||!h)return;const dpr=Math.min(2,window.devicePixelRatio||1);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);canvas.style.width=w+'px';canvas.style.height=h+'px'}const ctx=canvas.getContext('2d'),t=st.treeTransform;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);ctx.lineCap='round';
 graph.edges.forEach(edge=>{const a=graph.nodes.get(edge.from),b=graph.nodes.get(edge.to);if(!a||!b)return;const x1=(a.x+(edge.fromOffset||0))*t.scale+t.x,y1=(a.y+40)*t.scale+t.y,x2=(b.x+(edge.toOffset||0))*t.scale+t.x,y2=(b.y-42)*t.scale+t.y,minX=Math.min(x1,x2),maxX=Math.max(x1,x2),minY=Math.min(y1,y2),maxY=Math.max(y1,y2);if(maxX<-80||minX>w+80||maxY<-80||minY>h+80)return;const color=edge.kind==='material'?'#4ba890':edge.kind==='map'?'#739bc8':'#b59156',bend=Math.max(28,(y2-y1)*.48);ctx.strokeStyle=color;ctx.globalAlpha=.48;ctx.lineWidth=Math.max(1,1.6*t.scale);ctx.beginPath();ctx.moveTo(x1,y1);ctx.bezierCurveTo(x1,y1+bend,x2,y2-bend,x2,y2);ctx.stroke();ctx.globalAlpha=.82;ctx.fillStyle=color;ctx.beginPath();ctx.moveTo(x2,y2);ctx.lineTo(x2-4,y2-8);ctx.lineTo(x2+4,y2-8);ctx.closePath();ctx.fill()});ctx.globalAlpha=1;
}
function resetResearchTreeView(){const viewport=$('#researchTreeViewport');if(!viewport||!researchTreeGraph)return;const scale=Math.max(.015,Math.min(.82,(viewport.clientWidth-56)/researchTreeGraph.width));st.treeTransform={scale,x:(viewport.clientWidth-researchTreeGraph.width*scale)/2,y:34};st.treeInitialized=true;applyResearchTreeTransform()}
function setResearchTreeZoom(next,px,py){const viewport=$('#researchTreeViewport');if(!viewport||!researchTreeGraph)return;const old=st.treeTransform.scale;next=Math.max(.012,Math.min(2.2,next));px=Number.isFinite(px)?px:viewport.clientWidth/2;py=Number.isFinite(py)?py:viewport.clientHeight/2;st.treeTransform.x=px-(px-st.treeTransform.x)*(next/old);st.treeTransform.y=py-(py-st.treeTransform.y)*(next/old);st.treeTransform.scale=next;st.treeInitialized=true;applyResearchTreeTransform()}
function changeResearchTreeZoom(factor){setResearchTreeZoom(st.treeTransform.scale*factor)}
function renderResearchTree(){
 const view=$('#researchTreeView'),viewport=$('#researchTreeViewport'),stage=$('#researchTreeStage');if(!view||!viewport||!stage)return;$('#treeLegendResearch').textContent=treeText('research');$('#treeLegendMaterial').textContent=treeText('material');$('#treeLegendMap').textContent=treeText('map');$('#treeInteractionHint').textContent=treeText('hint');$('#researchTreeReset').textContent=treeText('reset');$('#researchTreeZoomIn').setAttribute('aria-label',treeText('zoomIn'));$('#researchTreeZoomIn').title=treeText('zoomIn');$('#researchTreeZoomOut').setAttribute('aria-label',treeText('zoomOut'));$('#researchTreeZoomOut').title=treeText('zoomOut');viewport.setAttribute('aria-label',treeText('aria'));
 researchTreeGraph=buildResearchTreeGraph();stage.style.width=researchTreeGraph.width+'px';stage.style.height=researchTreeGraph.height+'px';stage.innerHTML=[...researchTreeGraph.nodes.values()].map(researchTreeNodeHtml).join('');
 if(!researchTreeResizeObserver&&typeof ResizeObserver!=='undefined')researchTreeResizeObserver=new ResizeObserver(()=>drawResearchTreeEdges());if(researchTreeResizeObserver){researchTreeResizeObserver.disconnect();researchTreeResizeObserver.observe(viewport)}
 requestAnimationFrame(()=>{if(!st.treeInitialized)resetResearchTreeView();else applyResearchTreeTransform()});
}
function setResearchTreeMode(active){if(st.route!=='research'&&active)return;st.researchTree=!!active;st.treeInitialized=!active?false:st.treeInitialized;if(active)st.treeInitialized=false;renderSection()}
function researchTreeWheel(event){if(!st.researchTree||!researchTreeGraph)return;event.preventDefault();const viewport=$('#researchTreeViewport'),rect=viewport.getBoundingClientRect(),px=event.clientX-rect.left,py=event.clientY-rect.top;setResearchTreeZoom(st.treeTransform.scale*Math.exp(-event.deltaY*.0012),px,py)}
function researchTreePointerDown(event){if(!st.researchTree||event.button!==0||event.target.closest('[data-research-tree-id]'))return;const viewport=$('#researchTreeViewport');event.preventDefault();researchTreeDrag={id:event.pointerId,startX:event.clientX,startY:event.clientY,x:st.treeTransform.x,y:st.treeTransform.y};viewport.setPointerCapture(event.pointerId);viewport.classList.add('dragging')}
function researchTreePointerMove(event){if(!researchTreeDrag||event.pointerId!==researchTreeDrag.id)return;st.treeTransform.x=researchTreeDrag.x+event.clientX-researchTreeDrag.startX;st.treeTransform.y=researchTreeDrag.y+event.clientY-researchTreeDrag.startY;st.treeInitialized=true;applyResearchTreeTransform()}
function researchTreePointerUp(event){if(!researchTreeDrag||event.pointerId!==researchTreeDrag.id)return;const viewport=$('#researchTreeViewport');researchTreeDrag=null;viewport.classList.remove('dragging');if(viewport.hasPointerCapture(event.pointerId))viewport.releasePointerCapture(event.pointerId)}
...
function researchDiscountControl(x){const options=[[0,'baseCost'],[10,'discountOne'],[20,'discountTwo']];return '<div class="research-discount-control"><span><b>'+esc(ui('researchDiscount'))+'</b><small>'+esc(ui('researchDiscountDesc'))+'</small></span><div class="research-discount-options" role="group" aria-label="'+esc(ui('researchDiscount'))+'">'+options.map(option=>'<button type="button" data-research-discount="'+option[0]+'" data-research-id="'+x.id+'" class="'+(st.researchDiscount===option[0]?'active':'')+'">'+esc(ui(option[1]))+'</button>').join('')+'</div></div>'}
...
function researchRewards(x){const a=[];if(x.rewardEquipName)a.push(ico(x.rewardEquipIcon,'inline-card-icon',30)+rubyHtml(loc(x.rewardEquipName)));if(x.rewardSkillName)a.push(ico(x.rewardSkillIcon,'inline-card-icon',30)+rubyHtml(loc(x.rewardSkillName)));if(x.rewardItemName)a.push(ico(x.rewardItemIcon,'inline-card-icon',30)+rubyHtml(loc(x.rewardItemName)));(x.gainItems||[]).forEach(g=>{const id=Number(Array.isArray(g)?g[0]:g.itemId||g.id),n=Number(Array.isArray(g)?g[1]:g.amount||1),r=D.refs.items[id];a.push((r?ico(r.icon,'inline-card-icon',30)+rubyHtml(loc(r.name)):esc(tr('none')))+' &times; '+n)});if(x.gainGold)a.push(esc(tr('gold'))+' &times; '+x.gainGold);return a.length?'<div class="reward-list">'+a.map(v=>'<span>'+v+'</span>').join('')+'</div>':''}
...
function enemyModifierControls(x,modified,kind='enemies'){const toggle=(key,label,description)=>'<label class="enemy-modifier-toggle" title="'+esc(description)+'"><input type="checkbox" data-enemy-modifier="'+key+'" data-enemy-kind="'+kind+'" data-enemy-id="'+x.id+'"'+(st.enemyModifiers[key]?' checked':'')+'><span class="enemy-switch" aria-hidden="true"></span><b>'+esc(label)+'</b></label>';return '<div class="enemy-modifiers"><span class="enemy-modifier-title">'+esc(tr('customEnemyModifiers'))+'</span>'+toggle('strong',tr('strongTrial'),tr('strongTrialDesc'))+toggle('thorns',tr('thornJourney'),tr('thornJourneyDesc'))+(modified.summon?'<small>'+esc(tr('summonModifierExempt'))+'</small>':'')+'</div>'}
function bossLocationHtml(x){const points=x.bossLocations||[];return points.length?'<div class="boss-location-list">'+points.map(point=>'<div class="boss-location-row"><b>'+esc(loc(point.areaName))+'</b><span>'+esc(tr('coordinates'))+' '+point.x+', '+point.y+'</span></div>').join('')+'</div>':'<p>'+esc(tr('none'))+'</p>'}
function enemyDetail(x,isBossPage){const portrait=x.sprite?'<canvas class="enemy-sprite enemy-sprite-large" data-enemy="'+x.id+'" width="128" height="128"></canvas>':'<span class="detail-icon enemy-fallback">&#9822;</span>',modified=enemyModifiedStats(x),statValues=[['hp',modified.hp],['atk',modified.atk],['def',modified.def],['gold',x.gold]];if((x.categories||[x.category]).includes('elite')&&Number(x.escapeTurns)>0)statValues.push(['retreatTurns',x.escapeTurns]);const stats=statValues.map(v=>'<span'+(modified.boosted&&['hp','atk','def'].includes(v[0])?' class="modified-stat"':'')+'><small>'+esc(enemyStatLabel(v[0]))+'</small><b>'+v[1]+'</b></span>').join(''),abilities=x.abilities&&x.abilities.length?'<div class="ability-list">'+x.abilities.map(a=>'<article class="ability-card"><h4>'+esc(loc(a.name))+'</h4><p class="game-text">'+gameText(loc(a.description),{tags:{value:a.value}})+'</p></article>').join('')+'</div>':'<p>'+esc(tr('none'))+'</p>';return '<header class="detail-header">'+portrait+'<div><p class="detail-eyebrow">'+esc(tr(isBossPage?'bosses':'enemies'))+'</p><h2>'+esc(loc(x.name))+'</h2><div class="chip-row">'+(x.categories||[x.category]).map(category=>chip(tr(category),category==='boss'?'gold':category==='elite'?'orange':category==='summon'?'teal':'')).join('')+'</div></div></header>'+enemyModifierControls(x,modified,isBossPage?'bosses':'enemies')+'<div class="detail-stat-grid">'+stats+'</div>'+detailBlock(tr('abilities'),abilities)+detailBlock(tr('derivedEnemies'),enemyLinks(x.derivedEnemies))+detailBlock(tr('derivedFromEnemies'),enemyLinks(x.derivedFrom))+(isBossPage?detailBlock(tr('bossLocation'),bossLocationHtml(x)):detailBlock(tr('appearance'),'<div class="chip-row">'+x.maps.map(m=>chip(loc(m.name),'teal')).join('')+'</div>'))}
function openDetail(kind,id,option){const x=(D[kind]||[]).find(v=>v.id===Number(id));if(!x)return;setMobileFilters(false);const body=kind==='equipment'?equipmentDetail(x):kind==='skills'?skillDetail(x,option):kind==='research'?researchDetail(x,option):enemyDetail(x,kind==='bosses'),content=$('#detailContent');content.innerHTML=body;content.scrollTop=0;const dlg=$('#detailDialog');if(!dlg.open)dlg.showModal();requestAnimationFrame(drawEnemySprites)}
...
document.addEventListener('change',e=>{if(e.target.matches('[data-page-input]')){setPageFromInput(e.target);return}if(e.target===$('#saveFileInput')||e.target===$('#catalogSaveFileInput')){inspectSaveFile(e.target.files&&e.target.files[0]);e.target.value='';return}if(e.target.matches('[data-research-architect]')){const id=e.target.dataset.researchArchitect,level=$('[data-research-level]'),active=$('[data-research-cost-mode].active'),mode=active&&active.dataset.researchCostMode;st.architectMode=e.target.checked;openDetail('research',id,level&&level.value);if(mode==='range')setResearchCostMode('range');return}if(e.target.matches('[data-research-range-from],[data-research-range-to]')){updateResearchRange();return}if(e.target.matches('[data-enemy-modifier]')){st.enemyModifiers[e.target.dataset.enemyModifier]=e.target.checked;openDetail(e.target.dataset.enemyKind||'enemies',e.target.dataset.enemyId);return}if(e.target.matches('[data-filter]')){st.filters[e.target.dataset.filter]=e.target.value;st.page=1;renderSection();return}if(e.target===$('#sortSelect')){st.sort=e.target.value;st.page=1;renderSection();return}if(e.target.matches('[data-research-level]'))openDetail('research',e.target.dataset.researchLevel,e.target.value)});
