...
const st={lang:savedLang,route:'overview',query:'',statusQuery:'',filters:{},sort:'idAsc',visible:60,results:[],enemyModifiers:{strong:false,thorns:false},saveCheck:null};
...
const sectionData=k=>k==='all'?SECTIONS.flatMap(q=>D[q].map(x=>Object.assign({_kind:q},x))):(D[k]||[]);function searchable(x,k){
 ...
 if(k==='research')p.push(...(x.relatedItems||[]).map(c=>loc(c.name)),...(x.prerequisites||[]).map(v=>loc(v.name)),...(x.unlockItems||[]).map(v=>loc(v.name)),...(x.eventMaps||[]).map(v=>loc(v.name)));
...
function setRoute(k,o={}){
 ...
 st.route=k;st.visible=60;st.filters=o.filters||{};if(o.query!==undefined){st.query=o.query;$('#globalSearch').value=st.query}
...
function renderSaveCheckResult(){
 ...
 const acquired=new Set(result.acquiredIds),missing=(D.statuses||[]).filter(status=>!acquired.has(status.id));
...
function regularSaveStatusIds(fileName,raw){
 ...
 const ids=[];for(let id=1;id<=28;id++)if(record&&Object.prototype.hasOwnProperty.call(record,id)&&record[id]!==null&&record[id]!==undefined&&Number(record[id])>0)ids.push(id);
 return ids;
}
async function inspectSaveFile(file){
 if(!file)return;st.saveCheck=null;renderSaveCheckResult();
 if(file.size>20*1024*1024){st.saveCheck={kind:'invalid',readFailed:true};renderSaveCheckResult();return}
 try{const raw=await file.text(),ids=regularSaveStatusIds(file.name,raw);st.saveCheck=ids?{kind:'valid',fileName:file.name,acquiredIds:ids}:{kind:'invalid',readFailed:false};}
 catch(error){st.saveCheck={kind:'invalid',readFailed:true}}
 renderSaveCheckResult();
}
...
function card(x,k){
 ...
 if(k==='research')return '<article class="entry-card" data-kind="'+k+'" data-id="'+x.id+'"><div class="card-head">'+ico(x.icon)+'<div><h2>'+esc(loc(x.name))+'</h2><small>'+esc(tr('maxLevel'))+' '+x.maxLevel+'</small></div></div><p class="card-description">'+gameText(loc(x.description),x)+'</p><div class="chip-row">'+chip(typeName(x.type),'blue')+x.costs.slice(0,3).map(c=>chip(loc(c.name),'teal')).join('')+'</div></article>';
...
function renderSection(){
 ...
 $('#sectionEyebrow').textContent=k==='all'?'SEARCH':'DATABASE / '+k.toUpperCase();$('#sectionTitle').textContent=tr(tk);$('#sectionIntro').textContent=tr(ik);renderFilters(k);
 ...
 st.results=filtered(k);$('#resultCount').textContent=st.results.length;$('#catalog').innerHTML=st.results.slice(0,st.visible).map(x=>card(x,x._kind||k)).join('');$('#emptyState').hidden=st.results.length>0;$('#loadMore').hidden=st.visible>=st.results.length;requestAnimationFrame(drawEnemySprites);
}
...
function derivedHtml(v){if(!v.derived||!v.derived.length)return '';return detailBlock(tr('derived'),'<div class="derived-grid">'+v.derived.map(d=>'<article class="derived-card"><div class="derived-title">'+ico(d.icon,'inline-card-icon',34)+'<div><small>'+esc(d.kind==='equipment'?tr('equipment'):tr('skills'))+'</small><h4>'+esc(loc(d.name))+'</h4></div></div>'+(d.kind==='equipment'&&d.slots?'<div class="chip-row">'+chip(tr('slots')+' '+d.slots,'gold')+'</div>':'')+'<p class="game-text">'+gameText(loc(d.description),d)+'</p><div class="source-line"><b>'+esc(tr('acquisition'))+':</b> '+esc(loc(d.acquisition))+'</div></article>').join('')+'</div>')}
function skillDetail(x,tier){tier=Math.max(0,Math.min(Number(tier)||0,x.variants.length-1));const v=x.variants[tier],tabs=x.variants.length>1?'<div class="tier-tabs" role="tablist">'+x.variants.map((z,i)=>'<button type="button" class="tier-button'+(i===tier?' active':'')+'" data-skill-tier="'+i+'" data-skill-id="'+x.id+'" aria-label="'+esc(tr('level'))+' '+(i+1)+'">'+(i+1)+'</button>').join('')+'</div>':'';return '<header class="detail-header">'+ico(v.icon||x.icon,'detail-icon',64)+'<div class="detail-title-with-actor">'+actorImg(x.actorImage,'detail-actor')+'<div><p class="detail-eyebrow">'+esc(tr('skills'))+'</p><h2>'+esc(loc(x.name))+'</h2><p class="actor-name">'+esc(loc(x.actor))+'</p></div></div></header>'+tabs+detailBlock(tr('effect'),'<p class="game-text">'+gameText(loc(v.description),v)+'</p><div class="chip-row">'+metaChips(v)+'</div>')+(loc(x.flavor)?detailBlock(tr('flavor'),'<p>'+esc(loc(x.flavor))+'</p>'):'')+derivedHtml(v)+detailBlock(tr('acquisition'),'<p>'+esc(sourceText(x))+'</p>')}
...
function researchLevelPanel(x,level){const costs=researchCostAt(x,level),costHtml=costs.length?'<div class="cost-list">'+costs.map(c=>'<span>'+ico(c.icon,'inline-card-icon',30)+'<b>'+esc(loc(c.name))+'</b><em>&times; '+c.amount+'</em></span>').join('')+'</div>':'<p>'+esc(tr('noCost'))+'</p>';return '<div id="researchLevelBody"><p class="game-text">'+gameText(loc(x.description),x)+'</p>'+detailBlock(tr('levelCost'),costHtml)+'</div>'}
...
function researchDetail(x,level){const finite=x.maxLevel!=='\u221e'&&Number.isFinite(Number(x.maxLevel)),max=finite?Math.max(1,Number(x.maxLevel)):Infinity;level=Math.max(1,Math.floor(Number(level)||1));if(finite)level=Math.min(level,max);const picker=finite?'<select data-research-level="'+x.id+'">'+Array.from({length:max},(_,i)=>'<option value="'+(i+1)+'"'+(i+1===level?' selected':'')+'>'+esc(tr('level'))+' '+(i+1)+'</option>').join('')+'</select>':'<input type="number" min="1" step="1" value="'+level+'" data-research-level="'+x.id+'">';return '<header class="detail-header">'+ico(x.icon,'detail-icon',64)+'<div><p class="detail-eyebrow">'+esc(tr('research'))+'</p><h2>'+esc(loc(x.name))+'</h2><div class="chip-row">'+chip(typeName(x.type),'blue')+chip(tr('maxLevel')+' '+x.maxLevel,'gold')+'</div></div></header><div class="level-picker"><label>'+esc(tr('level'))+'</label>'+picker+'</div>'+researchLevelPanel(x,level)+detailBlock(tr('unlock'),researchUnlockHtml(x))+detailBlock(tr('reward'),researchRewards(x))+detailBlock(tr('relatedMaterials'),relatedMaterialsHtml(x))}
...
document.addEventListener('click',e=>{const lang=e.target.closest('[data-lang]');if(lang){st.lang=lang.dataset.lang;try{localStorage.setItem('rorWikiLang',st.lang)}catch(err){}applyLanguage();return}const tier=e.target.closest('[data-skill-tier]');if(tier){openDetail('skills',tier.dataset.skillId,tier.dataset.skillTier);return}const link=e.target.closest('[data-link-kind]');if(link){openDetail(link.dataset.linkKind,link.dataset.linkId);return}const route=e.target.closest('[data-route]');if(route){setRoute(route.dataset.route);return}const entry=e.target.closest('.entry-card');if(entry){openDetail(entry.dataset.kind,entry.dataset.id);return}const clear=e.target.closest('[data-clear]');if(clear){st.filters={};st.query='';$('#globalSearch').value='';renderSection();return}if(e.target===$('#loadMore')){st.visible+=60;renderSection();return}if(e.target===$('#mobileFilterButton')){$('#filters').classList.toggle('mobile-open');return}});
document.addEventListener('change',e=>{if(e.target===$('#saveFileInput')){inspectSaveFile(e.target.files&&e.target.files[0]);e.target.value='';return}if(e.target.matches('[data-enemy-modifier]')){st.enemyModifiers[e.target.dataset.enemyModifier]=e.target.checked;openDetail('enemies',e.target.dataset.enemyId);return}if(e.target.matches('[data-filter]')){st.filters[e.target.dataset.filter]=e.target.value;st.visible=60;renderSection();return}if(e.target===$('#sortSelect')){st.sort=e.target.value;renderSection();return}if(e.target.matches('[data-research-level]'))openDetail('research',e.target.dataset.researchLevel,e.target.value)});
document.addEventListener('input',e=>{if(e.target===$('#statusSearch')){st.statusQuery=e.target.value;renderStatusGrid();return}if(e.target===$('#globalSearch')){st.query=e.target.value;st.visible=60;if(st.route!=='overview'&&st.route!=='statuses')renderSection();return}if(e.target.matches('input[data-research-level]')){const id=e.target.dataset.researchLevel,value=e.target.value;if(value&&Number(value)>0)openDetail('research',id,value)}});
