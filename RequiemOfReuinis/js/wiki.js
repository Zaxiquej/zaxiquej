...
function researchRangeResult(x,fromLevel,toLevel){const finite=x.maxLevel!=='∞'&&Number.isFinite(Number(x.maxLevel)),cap=finite?Math.max(1,Number(x.maxLevel)):9999;fromLevel=Math.max(0,Math.min(Math.floor(Number(fromLevel)||0),cap));toLevel=Math.max(1,Math.min(Math.floor(Number(toLevel)||1),cap));if(toLevel<=fromLevel)return '<p class="research-range-error">'+esc(ui('invalidRange'))+'</p>';return '<p class="research-range-summary">'+esc(ui('currentLevel'))+' <b>'+fromLevel+'</b><span>→</span>'+esc(ui('targetLevel'))+' <b>'+toLevel+'</b></p>'+detailBlock(ui('totalCost'),researchCostList(researchRangeCosts(x,fromLevel,toLevel)))}
...
function researchDetail(x,level){const finite=x.maxLevel!=='\u221e'&&Number.isFinite(Number(x.maxLevel)),max=finite?Math.max(1,Number(x.maxLevel)):Infinity,rangeTarget=finite?max:1,rangeMax=finite?max:9999;level=Math.max(1,Math.floor(Number(level)||1));if(finite)level=Math.min(level,max);const picker=finite?'<select data-research-level="'+x.id+'">'+Array.from({length:max},(_,i)=>'<option value="'+(i+1)+'"'+(i+1===level?' selected':'')+'>'+esc(tr('level'))+' '+(i+1)+'</option>').join('')+'</select>':'<input type="number" min="1" step="1" value="'+level+'" data-research-level="'+x.id+'">',range='<div class="research-range-picker"><label><span>'+esc(ui('currentLevel'))+'</span><input type="number" min="0" max="'+Math.max(0,rangeMax-1)+'" step="1" value="0" data-research-range-from="'+x.id+'"></label><span aria-hidden="true">→</span><label><span>'+esc(ui('targetLevel'))+'</span><input type="number" min="1" max="'+rangeMax+'" step="1" value="'+rangeTarget+'" data-research-range-to="'+x.id+'"></label></div><div id="researchRangeBody">'+researchRangeResult(x,0,rangeTarget)+'</div>';return '<header class="detail-header research-detail-header">'+ico(x.icon,'detail-icon research-detail-icon',64)+'<div><p class="detail-eyebrow">'+esc(tr('research'))+'</p><h2>'+esc(loc(x.name))+'</h2><div class="chip-row">'+chip(typeName(x.type),'blue')+chip(tr('maxLevel')+' '+x.maxLevel,'gold')+'</div></div></header><p class="game-text research-description">'+gameText(loc(x.description),x)+'</p><div class="research-cost-calculator"><div class="research-cost-tabs" role="tablist"><button type="button" class="active" data-research-cost-mode="single" aria-selected="true">'+esc(ui('singleCost'))+'</button><button type="button" data-research-cost-mode="range" aria-selected="false">'+esc(ui('rangeCost'))+'</button></div><div data-research-cost-panel="single"><div class="level-picker"><label>'+esc(tr('level'))+'</label>'+picker+'</div>'+researchLevelPanel(x,level)+'</div><div data-research-cost-panel="range" hidden>'+range+'</div></div>'+detailBlock(tr('unlock'),researchUnlockHtml(x))+detailBlock(tr('reward'),researchRewards(x))+detailBlock(tr('relatedMaterials'),relatedMaterialsHtml(x))}
...
document.addEventListener('change',e=>{if(e.target.matches('[data-page-input]')){setPageFromInput(e.target);return}if(e.target===$('#saveFileInput')||e.target===$('#catalogSaveFileInput')){inspectSaveFile(e.target.files&&e.target.files[0]);e.target.value='';return}if(e.target.matches('[data-research-range-from],[data-research-range-to]')){updateResearchRange();return}if(e.target.matches('[data-enemy-modifier]')){st.enemyModifiers[e.target.dataset.enemyModifier]=e.target.checked;openDetail('enemies',e.target.dataset.enemyId);return}if(e.target.matches('[data-filter]')){st.filters[e.target.dataset.filter]=e.target.value;st.page=1;renderSection();return}if(e.target===$('#sortSelect')){st.sort=e.target.value;st.page=1;renderSection();return}if(e.target.matches('[data-research-level]'))openDetail('research',e.target.dataset.researchLevel,e.target.value)});
