...
function displayAbilities(c){
  const order={'法术':2,'入场曲':2,'爆能强化':2.5,'启动':3,'攻击时':3,'交战时':3.1,'在牌组中发动':1.1,'本卡牌被【瞬念召唤】时':1.2,'谢幕曲':4,'进化时':5,'本随从进化时':5.1,'超进化时':6};
  ...
  function effectParts(text){
    ...
    for(let i=0;i<sentences.length;i++){
      ...
      if(/^(?:若|如果|每当|每次|【(?:连击|协作|唤灵|土之秘术|觉醒|爆能强化))/.test(sentence)){
        const tail=sentences.slice(i).join(''),scope=tail.match(/^(?:\s*(?:若|如果)[^。]*?则|\s*【(?:连击|协作|唤灵|土之秘术|觉醒|爆能强化)[^】]*】)/);
      ...
      if(parts.length&&/^(?:X|Y|该|其|使其|这些|然后|否则|重复|此效果|此能力|此时|直到|并使|并将)/.test(sentence))parts[parts.length-1].text+=sentences[i];
      else parts.push({text:sentences[i]});
    }
    return parts;
  }
  let abilities=c.abilities.flatMap(a=>{
  ...
  for(const trigger of mergeable){
    ...
    const candidates=abilities.filter(a=>a.trigger===trigger&&!a.mode&&a.kind!=='mode'&&!a.text.includes('【模式】'));
    ...
    const conditional=a=>a.condition&&a.condition!=='none'||/若|如果|【(?:连击|协作|唤灵|土之秘术|觉醒)/.test(a.text.replace(/『[^』]*』/g,''));
...
function cardText(c){
  const parts=[`${c.name}\n${SVWB.CLASSES[c.class]} · ${SVWB.RARITIES[c.rarity]} · ${SVWB.TYPES[c.type]}\n${c.cost}费${c.type==='follower'?` ${c.attack}/${c.health}`:''}\n${displayAbilities(c).map(a=>a.text).join('\n')||'无能力'}`];
  if(c.tokens.length)parts.push('附属卡：\n'+c.tokens.map(t=>`${t.name}（${t.custom?'原创':'官方'}${t.tribe?' · '+t.tribe+'·随从':''}）\n${t.cost}费 ${t.attack}/${t.health}\n${t.text||'无能力'}`).join('\n\n'));
  if(c.emblems.length)parts.push('纹章：\n'+c.emblems.map(e=>e.name+'\n'+e.text).join('\n\n'));
  if(c.faiths.length)parts.push('信仰：\n'+c.faiths.map(f=>f.name+'\n'+f.text).join('\n\n'));
  if(c.alternateForms.length)parts.push('其他使用形态：\n'+c.alternateForms.map(f=>`${f.kind} ${f.cost}（${f.type}）\n${f.text}`).join('\n\n'));
  parts.push(`同人设计 · 规则 v${c.version}`);return parts.join('\n\n');
}
function render(){
  try{current=SVWB.generate($('name').value);}catch(e){$('status').textContent=e.message;$('name').focus();return;}
  const c=current;
  ...
  c.tokens.forEach(t=>{const box=el('div',undefined,'token');box.append(el('h4',t.name),el('div',`${t.custom?'原创 token':'官方卡牌'}${t.tribe?' · '+t.tribe+'·随从':''} · ${t.cost}费 · ${t.attack}/${t.health}`,'token-meta'),el('p',t.text||'无能力。'));if(!t.custom){const a=el('a','在官方图鉴中查看 ↗');a.href=`https://shadowverse-wb.com/chs/deck/cardslist/card/?card_id=${t.id}`;a.target='_blank';a.rel='noopener';box.append(a);} $('tokens').append(box);});
  $('emblems').replaceChildren();$('emblems').hidden=!c.emblems.length;
  if(c.emblems.length)$('emblems').append(el('h3','纹章'));
  c.emblems.forEach(e=>{const box=el('div',undefined,'token');box.append(el('h4',e.name),rich(e.text));$('emblems').append(box);});
  ...
  c.alternateForms.forEach(f=>{const box=el('div',undefined,'token');box.append(el('h3',`${f.kind}形态`),el('div',`${f.cost}费 · ${f.type}`,'token-meta'),rich(f.text),el('p',f.kind==='结晶'?'以护符使用时仅拥有以上能力；谢幕曲召唤本体不发动其入场曲。':'以法术使用时仅发动以上效果，不召唤本体。','token-meta'));$('alternate-forms').append(box);});
...
function filteredRandom(variant=false){
  try{$('name').value=SVWB.randomMatchingName(randomFilters(),{variantOf:variant?$('name').value:undefined,excludeName:$('name').value});render();if(variant)$('card').scrollIntoView({block:'nearest'});}
