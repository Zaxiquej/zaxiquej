(function (root) {
  ...
  // Soil is setup for a later paid Earth Rite, not immediate board value.
  // Only cheap followers' one-shot production gets this rate; recurring,
  // fusion, spells and emblem engines retain their full resource valuation.
  function earthUnitValue(type,cost,trigger){
    return type==='follower'&&cost>=1&&cost<=3&&['入场曲','谢幕曲'].includes(trigger)?(cost===1?1.1:.9):1.6;
  }
  const NAMED_BOARD_PAYOFFS={crystalHandBuff:'天晶魔手',experimentBuff:'沉溺的实验体',experimentGrant:'沉溺的实验体'};
  function producesNamedToken(node,name){
    return new RegExp(`(?:召唤[0-9X]+个|将[0-9X]+张)『${name}』`).test(node.bodyText||node.text||'');
  }
  function namedPayoffSupported(card,node,id){
    const name=NAMED_BOARD_PAYOFFS[id];
    if(!name)return true;
    // Other mode branches are alternatives, never guaranteed companion effects.
    return producesNamedToken(node,name)||card.abilities.some(a=>a!==node&&!a.modeBranches&&producesNamedToken(a,name));
  }
  function evolutionOnlyQuality(card){
    if(card.type!=='follower'||card.alternateForms?.length)return true;
    const effects=card.abilities.filter(a=>a.kind!=='keyword'&&a.kind!=='alternate');
    if(!effects.length||effects.some(a=>!['进化时','超进化时'].includes(a.trigger)))return true;
    const useful=a=>{
      if(a.modeBranches)return Math.max(...a.modeBranches.map(useful));
      if(a.ids.some(id=>NAMED_BOARD_PAYOFFS[id]&&!namedPayoffSupported(card,a,id)))return 0;
      return a.raw||0;
    };
    // EP-dependent bodies need a meaningful standalone payoff. A tribe name
    // cannot turn an empty-board buff into a reliable evolution reward.
    const minimum=4+card.cost*1.5+(card.rarity===3?2:0);
    return Math.max(...effects.map(useful))>=minimum;
  }
  function namedSupportQuality(card){
    const unsupported=card.abilities.some(a=>!a.modeBranches&&a.ids.some(id=>NAMED_BOARD_PAYOFFS[id]&&!namedPayoffSupported(card,a,id)));
    // Keep occasional deck-support cards; most named payoffs should bring their
    // own source. This is a design rejection, never a free appended producer.
    return !unsupported||hash(card.name+'|external-token-support')%7===0;
  }
  function lowSuperEvolutionQuality(card){
    if(card.type!=='follower'||card.cost<1||card.cost>3)return true;
    if(card.alternateForms?.length)return true;
    const effects=card.abilities.filter(a=>a.kind!=='keyword'&&a.kind!=='alternate');
    if(!effects.length||effects.some(a=>a.trigger!=='超进化时'))return true;
    // A cheap card saved until SEP must offer a substantial defining payoff,
    // not merely several small atoms or a plain stat boost.
    return effects.some(a=>a.raw>=10+card.cost&&(
      a.ids.some(id=>['selfCopy','teamEvolve','boardWipe','reanimate','artifactCopy','transformEnemy','transformEither','resurrectSelf','tripleAttack'].includes(id))||
      a.ids.includes('aoe')&&/造成[3-9]\d*点伤害/.test(a.text)||
      a.ids.some(id=>['tokenSummon','summon','experimentSummon','crystalHandSummon'].includes(id))&&/召唤[2-5]个/.test(a.text)||
      a.emblemIds?.some(id=>card.emblems.some(e=>e.id===id&&e.usableValue>=8))));
  }
  ...
  function highCostReadiness(card){
    ...
    for(const a of card.abilities){
      ...
      if(a.kind==='emblem'){
        ...
        const crests=(card.emblems||[]).filter(e=>a.emblemIds?.includes(e.id));
        const value=crests.reduce((sum,e)=>sum+(e.deliveryValue??Math.min(6,(e.effects||[]).reduce((n,p)=>n+p.raw,0))),0);
        score+=value*timing;continue;
      }
      ...
      const ids=a.ids.filter(id=>!['coreAnchor','coreSupport','highCostFloor','bodyPayoff','evolutionSacrifice'].includes(id));
  ...
  function transformationTaskValue(eventId,threshold,cost,rarity){
    // Design estimates: compare event opportunities, not raw counter numbers.
    const effort=threshold*({play:.55,rally:.65,spellboost:.65,deaths:.7,fusion:1.1,earth:1.5,enhance:1.8,evolutions:2,amuletDeath:1.4,hurt:.9,discard:1,leaderHurt:.9,artifacts:.9}[eventId]||1);
    return {effort,factor:Math.max(.28,Math.min(.55,.65-effort*.055)),minimumGain:3.5+effort*.7+(rarity===3?.75:0),effectCost:Math.min(10,cost+Math.ceil(effort/2))};
  }
  function lateGameQuality(card){
    if(card.type!=='follower'||card.cost<7||card.signature)return true;
    if(card.abilities.some(a=>a.ids.some(id=>['疾驰','handStorm','handDiscount','spellboostDiscount','costReduction','invocation','discardSelfSummon','progressTransform'].includes(id))))return true;
    return highCostReadiness(card)>=9+(card.cost-7)*3;
  }
  ...
  // Summoning an existing collectible does not trigger its Fanfare.
  TOKENS.push({id:10112130,name:'年幼宝石兽',class:1,cost:2,attack:2,health:2,text:'【入场曲】选择自己的战场上的1张其他卡牌，使其返回手牌。\n\n【超进化时】回复自己3点能量点。',poolWeight:1.2},
    {id:10011130,name:'温厚的树精',class:1,cost:4,attack:4,health:4,text:'【入场曲】【连击 3】本随从进化。\n\n【攻击时】回复自己的主战者2点生命值。',poolWeight:1});
  ...
  function stormCardValue(attack,health,abilities){
    const attacks=abilities.some(a=>!a.trigger&&a.ids.includes('tripleAttack'))?3:abilities.some(a=>!a.trigger&&a.ids.includes('doubleAttack'))?2:1;
    // Printed extra attacks are valued as total immediate attack damage below.
    // Do not also charge their standalone combat allowance a second time.
    const extras=abilities.filter(a=>!a.ids.includes('疾驰')&&!(!a.trigger&&a.ids.some(id=>['doubleAttack','tripleAttack'].includes(id))));
    ...
    const pressure=attack*attacks;
    const value=.3+pressure*.65+Math.max(0,pressure-2)**2*.07+health*.28+effectValue+Math.max(0,pressure-3)*faceDamage*.12;
    return {value,effectValue,faceDamage,attacks};
  }
  ...
  const TOKEN_VALUES = {
    ...
    90011310:[3.6,1.6],90031210:[1.6,2.8],90031310:[1.8,1.8],
  ...
  const freePersistentBody=t=>t.cost===0&&t.health>0&&!/破坏本卡牌|破坏本随从|消失/.test(t.text);
  function freeHandBoardValue(node){
    if(node.modeBranches)return Math.max(...node.modeBranches.map(freeHandBoardValue));
    return TOKENS.filter(freePersistentBody).reduce((sum,t)=>{
      const matches=[...(node.bodyText||node.text||'').matchAll(new RegExp(`将([1-9][0-9]*)张『${t.name}』加入手牌`,'g'))];
  ...
  function tokenValue(token,delivery='summon') {
    ...
    // Hand delivery must pay for its board value plus that storage flexibility.
    if(delivery==='hand'&&freePersistentBody(token))return Math.max(listed?.[0]||0,tokenValue(token,'summon')+.8);
  ...
  function tokenHandOffer(t,seed){
  ...
  const effects = [
    ...
    ['heal',1.4,6,[6,5],n=>`回复自己的主战者${n}点生命值。`,12],
  ...
  effects.push(
    ...
    {id:'tribeSupply',price:2.5,weight:4,classes:[1,2,3,4,5],exclusive:true,max:2,text:n=>`将${n}张『$TRIBETOKEN』加入手牌。`},
    {id:'tribeBuff',price:3.2,weight:4,classes:[1,2,3,4,5],exclusive:true,max:2,text:n=>`使自己的战场上的所有$TRIBE·随从+${n}/+${n}。`},
  ...
  function strategyTags(card) {
    const tags=new Set(),families={
      ...
      grave:/^(grave|necromancy|reanimate)/,ward:/^ward/,amulet:/^(amulet|activate|activeAmulet)/,
    ...
    for(const a of card.abilities||[]){
      ...
      const modal=a.kind==='mode'||a.mode||a.modeBranches;
      // Only requirements shared by every choice commit a modal card to a
      // strategy. Optional branches are not simultaneous deck requirements.
      if(modal){
        const branches=(a.modeBranches||[]).map(branch=>strategyTags({abilities:[branch]}));
        if(branches.length)for(const family of branches[0])if(branches.every(b=>b.includes(family)))tags.add(family);
      }else (a.ids||[]).map(id=>id.replace(/^fusionEvent:/,'')).forEach(tag);
      (a.fusionEvent?.effects||[]).forEach(tag);tag(a.condition);
      tag(a.engineSpec?.eventId);tag(a.progression?.eventId);
      if(a.trigger?.includes('天晶魔手'))tags.add('crystalHands');
      if(a.trigger?.includes('沉溺的实验体'))tags.add('experiment');
      if(a.trigger?.includes('土之秘术'))tags.add('earth');
      if(a.trigger?.includes('费用发生变化'))tags.add('truth');
      for(const t of modal?[]:a.tokens||[]){
  ...
  function strategyAffinity(context,candidate,alternate=false){
    ...
    if(disconnected.some(tag=>tag==='experiment'&&context.includes('crystalHands')||tag==='crystalHands'&&context.includes('experiment')))return alternate?.008:.025;
    if(disconnected.length)return alternate?.08:context.length>=2?.08:.22;
    return candidate.some(tag=>context.includes(tag))?2:1.4;
  }
  function designIdentity(card){
    const effects=card.abilities.filter(a=>a.kind!=='keyword'&&a.kind!=='alternate');
  ...
  function generate(input,options={}) {
    ...
    if(card.rarity===3&&!designIdentity(card)||!playable(card)){
      ...
      if(!playable(card)&&earlyFallback)card=earlyFallback;
    }
  ...
  function generateCard(input,alternateConfig=null,options={},skipProgression=false) {
    ...
    function effectAvailable(id,trigger){
      ...
      if(id==='selfCopy'&&used.has('resurrectSelf'))return false;
    ...
    function withStrategy(abilities,fn){
      pendingStrategy.push(strategyTags({abilities}));
      try{return fn();}finally{pendingStrategy.pop();}
    }
    function strategyWeight(candidate,extraContext=[]) {
      const context=[...new Set([...(modeChoiceDepth?[]:[...(alternateConfig?.strategyContext||[]),...strategyTags(card),...pendingStrategy.flat()]),...extraContext])];
      return strategyAffinity(context,strategyTags({abilities:[candidate]}),!!alternateConfig);
    }
    function synergyWeight(ids,tokens=[],trigger='',condition='none') {
      const contest=type==='follower'&&cost>=3&&cost<=6&&['进化时','超进化时'].includes(trigger);
      const tempoWeight=contest?(ids.some(id=>BOARD_EFFECTS.has(id))?1.7:ids.some(id=>['draw','tutor','keywordSearch','typeSearch','grave','earth'].includes(id))?.85:1):1;
      // The selected gate/event is already part of this node's plan, even
      // before add() commits it. Do not draw its payoff as an unrelated engine.
      const identity=type==='follower'&&cls!==0?CLASS_IDENTITY.families(cls,{ids,tokens,trigger,condition}):[];
      const identityPrior=CALIBRATION.classIdentityProfiles[`${cost<=3?0:cost<=6?1:2}:${cls}:${rarity}`];
      const prevalence=identity.length?Math.max(...identity.map(id=>identityPrior?.[id]||0)):0;
      // Prefer a class-appropriate atom, not an appended class package. Its
      // budget is unchanged, and strategyAffinity still rejects unrelated engines.
      const classWeight=identity.length?1+[.15,.4,.85,1.15][rarity]*(.6+1.5*Math.sqrt(prevalence)):1;
      const setupWeight=type==='follower'&&cost<=3&&ids.includes('earth')&&['入场曲','谢幕曲'].includes(trigger)?1.6:1;
      const summonWeight=ids.some(id=>['summon','tokenSummon'].includes(id))&&tokens.some(t=>SUMMON_FIRST.has(t.id))?1.4:1;
      let namedWeight=1;
      // A token definition attached to a buff is not a source of that token.
      // Bias both directions, while keeping separate modes independent.
      if(!modeChoiceDepth)for(const [payoff,name] of Object.entries(NAMED_BOARD_PAYOFFS)){
        const producer=card.abilities.some(a=>!a.modeBranches&&producesNamedToken(a,name));
        if(ids.includes(payoff))namedWeight=Math.max(.2,producer?2.5:.2);
        if(used.has(payoff)&&tokens.some(t=>t.name===name)){
          if(ids.some(id=>['summon','tokenSummon','crystalHandSummon','experimentSummon'].includes(id)))namedWeight=Math.max(namedWeight,3.5);
      ...
      const experimentLink=cls===3&&(used.has('experimentFusion')&&is('experimentSupply','experimentSummon','experimentBuff','experimentGrant')||(used.has('experimentLink')||crestTag('experiment')||hasCondition('experimentHistory')||used.has('experimentBuff')||used.has('experimentGrant'))&&experimentProducer||(used.has('experimentSummon')||used.has('experimentSupply')||alreadyProvides(10931110))&&is('experimentBuff','experimentGrant'));
      ...
      const match=
        ...
        card.abilities.some(a=>['攻击时','交战时'].includes(a.trigger))&&is('突进','doubleAttack','tripleAttack')||
    ...
    function enhanceSummons(candidate,t,count,limit,effectiveCost,complexity=null,trigger='入场曲'){
      const attackWindow=canGrantImmediateAttack(trigger);
      ...
      return {...candidate,text:candidate.text+text,raw:candidate.raw+extra,boardValue:(candidate.boardValue||candidate.raw)+extra,
        ...
        summonGrants:{trigger,tokenId:t.id,count,attack,health,attacks,effectiveCost,extra,grants:selected.map(g=>({id:g.id,raw:g.raw}))}};
    }
    const attackCount=()=>used.has('tripleAttack')?3:used.has('doubleAttack')?2:1;
    function repriceTripleAttack(){
      const a=card.abilities.find(a=>a.ids.includes('tripleAttack'));
      if(!a)return;
      const before=[card.attack,card.health];
      const reprice=()=>{const price=multipleAttackPrice(card.attack,3,a.trigger);card.spent+=price-a.price;a.price=price;a.raw=multipleAttackPrice(card.attack,3);};
      reprice();
      while(card.attack+card.health+card.spent>card.budget+1e-8&&card.health>1)card.health--;
      while(card.attack+card.health+card.spent>card.budget+1e-8&&card.attack>1){card.attack--;reprice();}
      card.bodyAllowance=card.attack+card.health;
      a.attackValuation={attack:card.attack,count:3};
      if(card.attack!==before[0]||card.health!==before[1])card.multiAttackBodyTrade={before,after:[card.attack,card.health]};
    }
  ...
  switch(kind) {
    ...
    case 'earth':{const x=rollAmount('earth',1,2,trigger);return atom(`使自己的战场上的土之印+${x}。`,x*earthUnitValue(type,cost,trigger));}
    ...
    case 'summon':{
      ...
      return enhanceSummons(result,t,count,Infinity,cost,null,trigger);
    }
  }
}
function pickAtom(limit,exclude=[],automatic=true,minRaw=0,trigger='入场曲') {
  ...
  if(wrapper==='phase') {
    ...
    const tailRaw=repeat?growth*2:healValue(cost,cost)+2.5;
    ...
    const timing=evolutionTiming('超进化时'),bonus=pickAtom((capacity-card.spent)/timing,[],true,evolutionPayoffFloor('超进化时',cost),'超进化时');
  ...
  return true;
    ...
    function composeEmblem(duration,allowLastWords=false,simple=false) {
      ...
      const [conditionId,conditionText,conditionFactor]=weighted(r,conditions.filter(c=>!simple||c[0]==='none').map(c=>[c,c[3]*strategyWeight({condition:c[0]},eventTags)]));
      const localTags=[...eventTags,...strategyTags({abilities:[{condition:conditionId}]})];
      ...
      const heal=clockEvent&&cost>=4?rollAmount('heal',1,Math.min(7,Math.ceil(cost*.65)),eventText):amount('heal'),damage=rollAmount('damage',clockEvent&&cost>=7?2:1,damageMax,eventText);
      part('draw',`抽取${!permanent&&cost>=7&&r()<.2?2:1}张卡牌。`,drawValue(cost,1),3);
      if(pool[0].text.includes('2张'))pool[0].raw=4.4;
      if(eventId!=='heal')part('heal',`回复自己的主战者${heal}点生命值。`,healValue(cost,heal),cls===6?5:3,[],['heal']);
      ...
      const buffSize=clockEvent&&cost>=7?3:cost>=6?2:1,[buffAttack,buffHealth]=growth(buffSize,'emblem');
      part('buff',`使自己的战场上的随机1个随从+${buffAttack}/+${buffHealth}。`,buffAttack+buffHealth,cls===1||cls===2?4:2);
      ...
      const supply=eventId==='experiment'?TOKENS[25]:eventId==='crystalHands'?TOKENS[16]:tribal?tribeToken:cls===7?artifactToken:cls===3?weighted(r,[...new Map([token,TOKENS[16],TOKENS[25]].map(t=>[t.id,t])).values()].map(t=>[t,strategyWeight({tokens:[t]},localTags)])):TOKENS[cls];
      const supplyOffer=handDelivery(supply);
      if(supplyOffer)part('supply',supplyOffer.text(1),supplyOffer.unitPrice,cls===0?1:4,[supply]);
      // Enter triggers cannot summon their own next event, even with a quota.
      if(!['artifact','crystalHands','experiment','tribeEnter'].includes(eventId)&&(supply.cost<=1||SUMMON_FIRST.has(supply.id)))part('summon',`召唤1个『${supply.name}』。`,tokenValue(supply),2,[supply],['enter']);
      ...
      // A slow clock needs a meaningful activation, especially behind another
      // condition. Rapid tribal triggers can retain modest per-event payoffs.
      const payoffFloor=clockEvent&&cost>=7?(3.5+(cost-7)*.4)/conditionFactor:0;
      const substantial=eligible.filter(p=>p.raw>=payoffFloor);
      const first=weighted(r,(substantial.length?substantial:eligible).map(p=>[p,p.weight*strategyWeight({ids:[p.id],tokens:p.tokens},localTags)]));
      ...
      if(!simple&&cost>=4&&r()<(permanent?.16:.4)){
        ...
        if(extras.length)parts.push(weighted(r,extras.map(p=>[p,p.weight*strategyWeight({ids:[p.id],tokens:p.tokens},[...localTags,...strategyTags({abilities:[{ids:[first.id],tokens:first.tokens}]})])])));
      }
      ...
      const enablers={experiment:{text:'将1张『沉溺的实验体』加入手牌。',raw:2,tokens:[TOKENS[25]]},fairy:{text:'将1张『妖精』加入手牌。',raw:1,tokens:[TOKENS[1]]},crystalHands:{text:'将1张『天晶魔手』加入手牌。',raw:1,tokens:[TOKENS[16]]},artifact:{text:'将1张『解析的创造物』加入手牌。',raw:1.7,tokens:[TOKENS[8]]},heal:{text:'回复自己的主战者2点生命值。',raw:healValue(cost,2),tokens:[]},earth:{text:'使自己的战场上的土之印+1。',raw:1.6,tokens:[]}};
      enablers.costChanged={text:'使自己的手牌中的随机1张随从的费用+1。',raw:-.35,tokens:[]};
      const tribeOffer=tribeToken?handDelivery(tribeToken):null;
      const enabler=['tribeEnter','tribeAttack'].includes(eventId)?(tribeOffer?{text:tribeOffer.text(1),raw:tribeOffer.unitPrice,tokens:[tribeToken]}:{text:'',raw:0,tokens:[]}):enablers[eventId]||{text:'',raw:0,tokens:[]};
      const eventRate=clockEvent?1:cost<=3?(limit===1?1.1:limit===2?1.8:['evolve','dragonEvolve','enhance'].includes(eventId)?2:3):limit===1?.85:limit===2?1.4:['evolve','dragonEvolve','enhance'].includes(eventId)?.8:1.6;
      const raw=cost<=3?Math.max(5,3+payoff*2.2*conditionFactor*eventRate):Math.max(3,1.5+payoff*2.2*conditionFactor*eventRate);
      const deliveryValue=payoff*conditionFactor*eventRate*(eventId==='start'?.6:1);
      const tokens=[...new Map([...parts.flatMap(p=>p.tokens),...enabler.tokens].map(t=>[t.id,t])).values()];
      return {text:`${eventText}，${eventId==='hurt'?'若为自己的回合，则':''}${quota}${conditionText}${parts.map(p=>p.text).join('')}`,eventText,conditionText,payoffText:parts.map(p=>p.text).join(''),raw,deliveryValue,conditionFactor,eventRate,tokens,enabler,eventId,conditionId,limit,effects:parts.map(p=>({id:p.id,raw:p.raw,produces:p.produces})),supportTags:[...new Set([eventId,conditionId])],engine:['crystalHands','experiment'].includes(eventId)?eventId:undefined};
    }
    function addEmblem() {
      ...
      const timings=[['入场曲',cost>=4?48:36,1],['进化时',36,evolutionTiming('进化时')],['超进化时',(cost<=3?5:16)*earlySuperWeight,evolutionTiming('超进化时')]];
      if(config.oneShot)timings.push(['谢幕曲',cls===5?36:20,.65]);
      const payoff=config.effects.reduce((sum,p)=>sum+p.raw,0);
      const usable=config.oneShot?payoff/(1+duration*.4):(config.deliveryValue||0)*2+Math.max(0,config.enabler.raw);
      if(cost>=7&&usable<5+(cost-7)*.5)return;
      const affordable=timings.filter(([t,,factor])=>(!(used.has('selfEvolve')||used.has('selfSuperEvolve'))||t==='入场曲')&&raw*factor<=budget-floor-card.spent&&(!['进化时','超进化时'].includes(t)||cost<3||usable>=evolutionPayoffFloor(t,cost)));
      if(!affordable.length)return;
      const [trigger,,factor]=weighted(r,affordable.map(t=>[t,t[1]]));
      const emblem={id:'emblem-'+cls,name:`纹章：${name}`,kind:'emblem',class:cls,custom:true,duration,engine:config.engine,eventId:config.eventId,conditionId:config.conditionId,limit:config.limit,effects:config.effects,supportTags:config.supportTags,deliveryValue:config.oneShot?payoff/(1+duration*.7):config.deliveryValue,usableValue:usable,oneShot:!!config.oneShot,text:(duration===null?'':`【吟唱 ${duration}】\n`)+config.text};
    ...
    function addFaith() {
      ...
      const trigger=weighted(r,[['入场曲',4],['进化时',4],['超进化时',2*earlySuperWeight]]);
  ...
  const timing=trigger==='入场曲'?1:evolutionTiming(trigger);
  const minimum=Math.max(8-card.abilities.reduce((s,a)=>s+a.raw,0),evolutionPayoffFloor(trigger,cost));
  const support=card.abilities.length<3&&(!main||main.raw<cost*1.8)?pickAtom(Math.min(20,(budget-floor-card.spent)/timing),main?main.ids:[],trigger!=='入场曲',Math.max(5,minimum),trigger):null;
  ...
  if(card.corePower>=8&&!card.abilities.some(a=>a.major))card.abilities.reduce((a,b)=>a.raw>=b.raw?a:b).major=true;
    ...
    function addSacrificeDesign(){
      if(options.earlyPlayFocus||cost<2||cost>9||oversized||vanilla||card.abilities.some(a=>a.kind!=='keyword')||used.has('疾驰'))return false;
      ...
      if(discard){
        ...
        const loss=Math.min(floor-2,2+Math.floor(sr()*Math.max(1,floor*.5))),target=floor-loss;if(loss<2)return false;
        ...
        const payoff=withBlocked(['selfCopy','buff',...Object.keys(NAMED_BOARD_PAYOFFS)],()=>makeEffect(limit,trigger,'none',minimum,true,Math.min(10,cost+Math.ceil(loss/2)),false,{maxAtoms:rarity===0?1:2}));
        if(!payoff)return false;
    ...
    function unlockGate(id,trigger,effectiveCost=cost){
      ...
      const lateEvolution=superUnlock&&trigger==='进化时';
      return {id,text:`若为${superUnlock?'超进化':'进化'}已解禁的回合，则`,factor:1-(1-base)/(1+repeats),extra:0,
        minRaw:lateEvolution?6+Math.max(0,delay)*.6:2,effectBoost:lateEvolution?2:0};
    }
    function gate(trigger) {
      ...
      // Avoid introducing a separate deck requirement late in an established
      // plan. Rejected gates receive neither their discount nor cost credit.
      if(rng(hash(seed+'|strategy-gate|'+trigger+'|'+card.abilities.length))()>Math.min(1,strategyWeight({condition:selected.id})))return {id:'none',text:'',factor:1,extra:0};
    ...
    function makeEffect(maxPrice,trigger,condition,minRaw=0,allowResourceSupport=true,effectCost=cost,blockTarget=false,complexity=null,allowedIds=null) {
      ...
      if(trigger==='本卡牌被舍弃时'){effectCost=2;maxPrice=Math.min(maxPrice,3.5);}
      alignTribeToken();
      const handOffer=handDelivery(token);
      const tribeHandOffer=tribeToken?handDelivery(tribeToken):null;
      ...
      const hasAdvantage=cheapBase&&card.abilities.some(a=>(a.condition==='none'||a.condition==='discard')&&['入场曲','谢幕曲','法术'].includes(a.trigger)&&(
        a.ids.some(id=>advantageIds.includes(id))||a.ids.includes('commonSupply')&&a.tokens?.some(t=>t.cost===0)||a.emblemIds?.some(id=>card.emblems.find(e=>e.id===id)?.effects.some(e=>['draw','tutor','supply'].includes(e.id)))
      ));
      const pool=effects.filter(e=>(e.id!=='grantRush'||canGrantImmediateAttack(trigger))&&(!allowedIds||allowedIds.includes(e.id))&&effectAvailable(e.id,trigger)&&(!e.types||e.types.includes(type))&&(!e.exclusive||e.classes.includes(cls))&&
        !(condition==='superUnlocked'&&trigger==='进化时'&&e.id==='buff')&&
        ...
        !(e.id==='commonSupply'&&commonSupply?.cost===0&&hasAdvantage)&&
      ...
      for(const e of pool) {
        ...
        if(e.id==='earth')price=earthUnitValue(type,effectCost,trigger);
        const healingCost=type==='amulet'&&trigger==='谢幕曲'?cost:effectCost;
        if(e.id==='heal')price=healValue(healingCost,1);
        ...
        if(e.id==='tribeSupply'){tokens=[tribeToken];price=tribeHandOffer.unitPrice;}
        if(e.id.startsWith('crystalHand'))tokens=[TOKENS[16]];
        if(e.id.startsWith('experiment'))tokens=[TOKENS[25]];
        const freeHandBody=['tokenHand','tribeSupply'].includes(e.id)&&tokens.some(freePersistentBody);
        // An unconditional one-PP follower must not also bank a free combo play.
        // Timing/condition discounts remain available for delayed or gated rewards.
        if(type==='follower'&&effectCost<=1&&trigger==='入场曲'&&condition==='none'&&
          ['commonSupply','tokenHand','tribeSupply'].includes(e.id)&&tokens.some(t=>t.class===1&&t.cost===0))continue;
        ...
        const summonTempo=effectCost>=2&&effectCost<=3&&['入场曲','法术'].includes(trigger)&&
          (freeHandBody||['tokenSummon','fusionArtifactSummon','crystalHandSummon','experimentSummon'].includes(e.id));
        ...
        if(e.id==='heal'){
          const healCap=healingCost<=2?healingCost+1+(evolved?2:0):Math.min(12,healingCost+2);
          max=Math.min(max,healCap);while(max>0&&healValue(healingCost,max)>maxPrice)max--;
        }
        if(search){
        ...
        let candidate={ids:[e.id],raw:['recoverFollower','artifactRecover'].includes(e.id)?price*n+.6*n*(n-1):handCycle?cycleValue(n):e.id==='heal'?healValue(healingCost,n):e.id==='draw'?drawValue(effectCost,n):search?searchValue(e.id,n):copyIds.has(e.id)?copyValue(e.id,n):summonCost(n)+overhead,boardValue:developsBoard?summonCost(n):0,text,tokens};
        if(freeHandBody)candidate.boardValue=n*tokenValue(tokens[0])+summonPremium(n);
        ...
        if(['tokenSummon','fusionArtifactSummon','crystalHandSummon','experimentSummon'].includes(e.id))candidate=enhanceSummons(candidate,tokens[0],n,maxPrice,effectCost,complexity,trigger);
    ...
    function addMidEvolutionCore(){
      ...
      const effect=makeEffect(Math.min(room*.72/timing,2*cost+10),trigger,'none',min,false,cost,false,{maxAtoms:rarity===0?1:2},allowed);
      if(!effect)return;
      add({...effect,kind:'effect',trigger,condition:'none',bodyText:effect.text,text:`【${trigger}】`+effect.text,price:effect.raw*timing,midEvolutionCore:true});
    }
    function addRareCombatModules(){
      ...
      for(let pass=0;pass<2&&card.abilities.length<4;pass++){
        ...
        const choices=[];
        const offer=(id,text,raw,trigger='',factor=1,ids=[id],extra={})=>{
          const price=raw*factor;
          if(!used.has(id)&&price+(extra.repeatPremium||0)+(extra.keywordPremium||0)<=budget-floor-card.spent)choices.push({kind:trigger?'effect':'static',trigger,condition:'none',bodyText:text,text:(trigger?`【${trigger}】`:'')+text,raw,price,ids,tokens:[],...extra});
        };
        ...
        if(highRole!=='defense'&&cost>=7&&rarity===3&&!used.has('handStorm')&&!used.has('doubleAttack')&&!used.has('tripleAttack')&&(!used.has('疾驰')||cost>=8)){
          const trigger=used.has('疾驰')||used.has('selfEvolve')||used.has('selfSuperEvolve')?'':weighted(roll,[['',4],['进化时',2],['超进化时',4]]);
          const timing=trigger==='超进化时'?.5:trigger==='进化时'?.7:1;
          const repeatPremium=card.abilities.filter(a=>['攻击时','交战时'].includes(a.trigger)).reduce((sum,a)=>sum+a.price,0)*2;
          const estimate=card.stormPlan?.attack||Math.max(1,Math.round(floor*(.5+attackBias)));
          const keywordReprices=card.abilities.filter(a=>a.kind==='keyword'&&a.ids.some(id=>['疾驰','虹吸','屏障','潜行'].includes(id))).map(a=>({a,price:keywordPrice(a.ids[0],estimate,Math.max(1,floor-estimate),{attacks:3,ward:used.has('守护')})}));
          const keywordPremium=keywordReprices.reduce((sum,v)=>sum+Math.max(0,v.price-v.a.price),0);
          // Timing discounts the attack grant, never the existing repeat engines.
          offer('tripleAttack',trigger?'本随从获得「1回合可以攻击3次」。':'1回合可以攻击3次。',multipleAttackPrice(estimate,3),trigger,timing,['tripleAttack'],{repeatPremium,keywordPremium,keywordReprices,weight:2});
        }
        if(!choices.length)break;
        const selected=weighted(roll,choices.map(a=>[a,a.weight||1]));
        if(selected.ids.includes('tripleAttack')){
          for(const a of card.abilities.filter(a=>['攻击时','交战时'].includes(a.trigger))){
            a.singleAttackPrice=a.price;card.spent+=a.price*2;a.price*=3;a.attackMultiplicity=3;
          }
          for(const {a,price} of selected.keywordReprices){const next=Math.max(a.price,price);card.spent+=next-a.price;a.price=a.raw=next;}
          delete selected.keywordReprices;
        }
        add(selected);
      }
    }
    function addHandCostAndDiscard(){
      ...
      // Discarding does not pay the printed PP cost. Ordinary rewards use the
      // same small allowance at every cost; delivery of this card is separately valued.
      if(type==='follower'&&cost<=4&&roll()<.35&&room+2>=4){
        ...
        const e=makeEffect(Math.min(3.5,room/1.25),'本卡牌被舍弃时','none',.8,true,2);
    ...
    function addProgressTransform(){
      ...
      const capacity=room/factor,plans=[];
      const baseAbilities=card.abilities.map(a=>({...a}));
      const wrap=(effect,trigger,price=effect.raw)=>({...effect,kind:'effect',trigger,condition:'none',price,
        text:(trigger==='法术'?'':trigger.includes('回合')?trigger+'，':'【'+trigger+'】')+effect.text,bodyText:effect.text});
      const offer=(style,abilities,raw,bonusRaw,discount,weight,tokens=[],changedIndex=null)=>{
        if(raw>=task.minimumGain&&raw-discount*2.4>=task.minimumGain*.75&&raw*factor<=room)plans.push([{style,abilities,raw,bonusRaw,discount,tokens,changedIndex},weight]);
      };
      const trigger=type==='spell'?'法术':'入场曲';
      // A lower PP cost is one optional direction, not part of every upgrade.
      const discount=weighted(pr,[[0,5],[1,3],[2,cost>=4&&rarity>=2?1:0]]),discountRaw=discount*2.4;
      const ceiling=Math.min(12,(capacity-discountRaw)/replayMultiplier);
      const bonus=ceiling>=task.minimumGain*.75?makeEffect(ceiling,trigger,'progression',Math.max(task.minimumGain*.75,(task.minimumGain-discountRaw)/replayMultiplier),true,task.effectCost,false,{maxAtoms:2},['damage','aoe','heal','draw','tokenSummon','tokenHand','boost','earth','grave','experimentSummon','crystalHandSummon','teamBuff','artifactBuff']):null;
      if(bonus)offer('augment',[...baseAbilities,wrap(bonus,trigger)],discountRaw+bonus.raw*replayMultiplier,bonus.raw,discount,2,bonus.tokens);
      for(let i=0;i<baseAbilities.length;i++){
        const a=baseAbilities[i];
        if(a.condition!=='none'||a.ids.length!==1||a.kind==='keyword'||a.kind==='replay')continue;
        const replay=a.trigger==='入场曲'?replayMultiplier:1;
        const timing=a.raw>0?a.price/a.raw:1;
        const saved=card.abilities;let stronger;
        try{
          card.abilities=saved.filter((_,j)=>j!==i);refreshUsed();
          stronger=makeEffect(a.raw+capacity/(timing*replay),a.trigger,'progression',a.raw+task.minimumGain/(timing*replay),true,task.effectCost,false,{maxAtoms:1},a.ids);
        }finally{card.abilities=saved;refreshUsed();}
        if(stronger){
          const delta=(stronger.raw-a.raw)*timing*replay;
          offer('amplify',baseAbilities.map((old,j)=>j===i?wrap(stronger,a.trigger,stronger.raw*timing):old),delta,delta/replayMultiplier,0,3,stronger.tokens,i);
        }
        if(type==='follower'&&['进化时','超进化时'].includes(a.trigger)&&!phaseHasTarget('入场曲')){
          const delta=Math.max(0,a.raw-a.price)+a.raw*(replayMultiplier-1);
          const promoted={...a,trigger:'入场曲',price:a.raw,text:'【入场曲】'+a.bodyText};
          offer('promote',baseAbilities.map((old,j)=>j===i?promoted:old),delta,delta/replayMultiplier,0,3,a.tokens||[],i);
        }
      }
      if(type==='follower'){
        const frail=card.health<=2&&!card.abilities.some(a=>a.ids.some(id=>['突进','屏障','damageCap'].includes(id)));
        const event=weighted(pr,[['攻击时',frail?0:3],['交战时',frail?0:3],['自己的回合结束时',2]]);
        const repeats=card.health<=2?1:event==='交战时'?1.6:1.25;
        const payload=makeEffect(Math.min(14,capacity/repeats),event,'progression',task.minimumGain/repeats,true,task.effectCost,false,{maxAtoms:1},['damage','heal','boost','earth','grave','tokenSummon','experimentSummon','crystalHandSummon']);
        if(payload)offer('engine',[...baseAbilities,wrap(payload,event,payload.raw*repeats)],payload.raw*repeats,payload.raw*repeats/replayMultiplier,0,3,payload.tokens);
      }
      if(!plans.length)return;
      const plan=weighted(pr,plans),raw=plan.raw,price=raw*factor;
      ...
      const upgraded={id:'upgrade-'+seed,name:name+'·完成形',class:cls,type,cost:cost-plan.discount,attack:card.attack||0,health:card.health||0,custom:true,upgrade:true,
        text:plan.abilities.map(a=>a.text).join('\n\n'),abilities:plan.abilities};
      const counter=`使本卡牌的X+1。之后，若X为${threshold}或以上，则本卡牌变身为『${upgraded.name}』。`;
      const text='X起始为0。\n'+(eventId==='spellboost'?event:'在手牌中发动。'+event+'，')+counter;
      const progression={eventId,threshold,task,targetId:upgraded.id,style:plan.style,changedIndex:plan.changedIndex,discount:plan.discount,discountRaw:plan.discount*2.4,bonusRaw:plan.bonusRaw,replayMultiplier,price,factor,baseText};
      add({kind:'progressTransform',trigger:eventId==='spellboost'?'魔力增幅时':'在手牌中发动',condition:'progression',text,bodyText:text,raw,price,ids:['progressTransform'],progression,tokens:[upgraded,...plan.tokens]});
    ...
    function addDrawLuck(){
      ...
      const trigger=type==='spell'?'法术':weighted(lr,[['入场曲',6],['进化时',type==='follower'?2:0],['超进化时',type==='follower'?2*earlySuperWeight:0]]);
      ...
      if(cls===5){
        ...
        const payoff=makeEffect(Math.min(cost>=7?12:6,room),'谢幕曲','drawCost',cost>=7?4+cost*.25:2,false,cost,false,null,safeIds);
        ...
        const deliveryValue=Math.max(0,payoff.raw*probability+missRaw*(1-probability)-downsideValue*(1-probability)*.4)*1.5;
        if(['进化时','超进化时'].includes(trigger)&&deliveryValue*2<evolutionPayoffFloor(trigger,cost))return;
        ...
        card.emblems.push({id,name:crestName,kind:'emblem',class:cls,custom:true,duration,eventId:'draw',conditionId:'drawCost',limit:null,supportTags:['drawLuck'],deliveryValue,usableValue:deliveryValue*2,effects:[{id:payoff.ids[0],raw:payoff.raw,produces:[]},...(missEffect?[{id:missEffect.ids[0],raw:missEffect.raw,produces:[]}]:[])],text,luck});
    ...
    function addRareInteraction(extensionOnly=false){
      ...
      if(roll()>(extensionOnly?.18:rarity===3?.65:chaos?.25:.16))return;
      ...
      const trigger=type==='spell'?'法术':weighted(roll,[['入场曲',6],['进化时',type==='follower'?2:0],['超进化时',type==='follower'?earlySuperWeight:0]]);
      const timing=['进化时','超进化时'].includes(trigger)?evolutionTiming(trigger):1;
      let e=makeEffect((budget-floor-card.spent)/timing,trigger,'none',0,true,cost,false,null,ids);
      // Scale the target count of an independently selected control atom.
      // One selection of two enemies is still one targeted ability.
      if(e&&trigger==='超进化时'&&cost>=4&&e.ids.length===1&&['silence','setHealth',...(cost>=7?['transformEnemy']:[])].includes(e.ids[0])){
        const raw=e.raw*1.8;
        if(raw*timing<=budget-floor-card.spent)e={...e,raw,text:e.text.replace('1个随从','2个随从'),targetCount:2};
        else if(cost>=7)return;
      }
    ...
    function addResurrection(){
      if(options.skipResurrection||type!=='follower'||![3,5].includes(cls)||cost<4||vanilla||card.abilities.length>2)return;
      const rr=rng(hash(seed+'|resurrection'));
      if(rr()>(cls===5?[.035,.08,.12,.17][rarity]:rarity>=2?.055:.01))return;
      const material=cls===3?'earth':rr()<.5?'selfDamage':'necromancy';
      const amount=material==='selfDamage'?2+Math.floor(rr()*3):material==='earth'?2+Math.floor(rr()*2):4+Math.floor(rr()*3);
      const credit=material==='selfDamage'?amount*.65:material==='earth'?amount*.8:amount*.3;
      const body=floor*.8,retained=card.abilities.filter(a=>a.trigger!=='入场曲'&&a.kind!=='alternate').reduce((n,a)=>n+a.price,0);
      const raw=(body+retained*.7+2)*1.35,price=Math.max(3,raw-credit);
      if(price>budget-floor-card.spent)return;
      const summon=`召唤1个『${name}』。`;
      const text=material==='selfDamage'?summon+`对自己的主战者造成${amount}点伤害。`:`【${material==='earth'?'土之秘术':'唤灵'} ${amount}】`+summon;
      add({kind:'resurrection',trigger:'谢幕曲',condition:material,resourceCost:amount,text:'【谢幕曲】'+text,bodyText:text,raw,price,ids:['resurrectSelf'],tokens:[],resurrection:{material,amount,credit,body,retained,repeats:1.35},major:cost>=6});
    }
    ...
    function addExhaustibleCycle(){
      ...
      const payloadTrigger=type==='amulet'?'自己的回合开始时':'自己的回合结束时';
      for(let i=0;i<3;i++){
        const p=withStrategy(parts,()=>withBlocked(parts.flatMap(p=>p.ids),()=>pickAtom(limit,['pp','teamBuff','reanimate'],true,1.5,payloadTrigger)));
      ...
      const protectedBody=['潜行','威慑','屏障','damageCap','abilityDestructionImmune'].some(id=>used.has(id));
      const trigger=type==='amulet'||protectedBody?'自己的回合开始时':'自己的回合结束时';
      const text=trigger+'，从以下未发动的能力中随机发动1个能力。\n'+parts.map((p,i)=>'（'+(i+1)+'）'+p.text).join('\n');
      add({kind:'cycle',trigger,condition:'none',text,bodyText:text,raw,price,ids:['exhaustibleCycle',...parts.flatMap(p=>p.ids)],tokens:parts.flatMap(p=>p.tokens),cycle:{withoutReplacement:true,reset:false,branches:parts.map(p=>({text:p.text,raw:p.raw,ids:p.ids}))}});
    }
    ...
    function addExpandedSystems(){
      ...
      if([1,4,5,7].includes(cls)&&cost>=4&&rarity>=2&&card.abilities.length<3&&roll()<.16){
        const duration=pick([2,3]),event=pick(['start','end','enter']),amount=cost>=7?(event==='enter'?(roll()<.4?2:1):weighted(roll,[[2,3],[3,1]])):1;
        const poison=roll()<.5;
        ...
        if(price<=budget-floor-card.spent){
          const emblem={id:'enemy-emblem',name:`纹章：${name}的咒缚`,class:cls,kind:'emblem',custom:true,owner:'opponent',duration,eventId:event,conditionId:'none',limit:null,supportTags:[],deliveryValue:raw*(event==='enter'?1.5:1),effects:[{id:poison?'leaderDamage':'statDebuff',raw,produces:[]}],text:`【吟唱 ${duration}】\n${trigger}，${payload}`};
    ...
    if(type!=='follower'){
      ...
      NONFOLLOWERS.build({card,cost,cls,rarity,r,pick,weighted,profile,makeEffect,gate,composeEmblem,add,used,withBlocked,withStrategy,withModeChoice,tokenValue,canChooseTarget,calibration:CALIBRATION,alternate:!!alternateConfig,reserveProgress});
    ...
    // The high-cost core reserves one meaningful payoff instead of appending a role kit.
    ...
    tradeBodyForPower();
    addSpecialMechanic();
    const plannedCount=signature||simpleDesign?card.abilities.length:vanilla?0:(cost>=6?Math.max(card.abilities.length,rarity>=2?3:2):Math.max(card.abilities.length,Math.min(4,1+(r()<.55+rarity*.12?1:0)+(cost>=4&&r()<.4+rarity*.12?1:0))));
    ...
    for(let i=card.abilities.length;i<count;i++) {
      ...
      const links=cls===6?[["自己的原始费用为6或以上的其他卡牌进入战场时",.035],['自己的拥有【守护】的其他随从被破坏时',.035],['自己【启动】护符时',.055*(used.has('activeAmuletRecruit')||used.has('activeAmuletSearch')?3:1)],['自己的护符被破坏时',.03]]:cls===7?[["自己的原始费用为5或以上的其他随从进入战场时",.05],['自己的其他创造物·随从进入战场时',.055]]:cls===2?[['自己的其他随从进入战场时',.025]]:cls===3?[['自己使用费用发生变化的随从时',.05],["自己的『天晶魔手』进入战场时",.05*(used.has('crystalHandSummon')||used.has('crystalHandSupply')?2:1)],["自己的『沉溺的实验体』进入战场时",.035*(used.has('experimentSupply')||used.has('experimentSummon')?2:1)]]:cls===4&&floor>=4?[['本随从受到伤害且没被破坏时',used.has('allyPing')||used.has('allBoardDamage')?.14:.07]]:[];
      const combatWeight=cost<2?0:(cls===3)&&cost>=3?.05:.012;
      let trigger=weighted(r,[...['入场曲','进化时','超进化时','谢幕曲','攻击时'].map(t=>[t,triggerWeight(t)]),['交战时',combatWeight],...links.map(([t,w])=>[t,w*strategyWeight({trigger:t})]),['自己的回合结束时',.025],['爆能强化',cost<10?profile.triggers['爆能强化']*synergyWeight(['enhance']):0]]);
      ...
      const evolutionPayoff=evolutionPayoffFloor(trigger,cost);
      ...
      if(!(cost<=2&&rarity===0)&&(g.difficult&&g.minRaw>=6||g.fee>=7)&&(!e||r()<(g.fee ? .72 : .55))) {
        ...
        if(first) {
          const second=withStrategy([first],()=>withBlocked(first.ids,()=>effect(rawLimit-first.raw,Math.max(0,minRaw-first.raw),true,targeted(first.text))));
      ...
      const linkId=trigger.includes('天晶魔手')?'crystalHandLink':trigger.includes('沉溺的实验体')?'experimentLink':trigger.includes('受到伤害')?'hurtLink':trigger.includes('费用发生变化')?'costChangedLink':trigger.includes('拥有【守护】')?'wardLink':trigger.includes('护符')?'amuletLink':trigger.includes('创造物')?'artifactLink':trigger==='自己的其他随从进入战场时'?'rallyLink':null;
      if(linkId&&used.has(linkId))continue;
      if(linkId==='crystalHandLink')e.tokens=[...e.tokens,TOKENS[16]];
      if(linkId==='experimentLink')e.tokens=[...e.tokens,TOKENS[25]];
    ...
    if(!vanilla&&!card.abilities.some(a=>a.trigger&&a.kind!=='alternate')) {
      ...
      const triggers=['入场曲','谢幕曲','进化时'];
      const candidates=triggers.map(trigger=>{const timing=trigger==='进化时'?evolutionTiming(trigger):trigger==='谢幕曲'?.65:1;
        const e=makeEffect((budget-floor-card.spent)/timing,trigger,'none',.7,false,cost,false,{simple:rarity===0,maxAtoms:1});
        return e?{...e,kind:'effect',trigger,condition:'none',price:e.raw*timing,text:'【'+trigger+'】'+e.text,bodyText:e.text}:null;
      }).filter(Boolean);
      if(candidates.length)add(weighted(r,candidates.map(a=>[a,triggerWeight(a.trigger)])));
    }
    ...
    if(evolutions.length&&!hasOtherPayoff&&!card.evolutionBodyTrade) {
      ...
      // Improve the existing numeric effect instead of appending an unrelated
      // draw/heal atom. Compound, conditional and emblem abilities keep their
      // semantics; they are not flattened merely to fill a strength quota.
      if(target.kind==='effect'&&target.condition==='none'&&target.ids.length===1){
        const saved=card.abilities;card.abilities=saved.filter(a=>a!==target);refreshUsed();
        const stronger=makeEffect(target.raw+limit,target.trigger,'none',target.raw+Math.min(1,min),false,cost,false,{maxAtoms:1},target.ids);
        card.abilities=saved;refreshUsed();
        if(stronger&&stronger.raw>target.raw){
          const delta=stronger.raw-target.raw,price=delta*timing,oldTokens=target.tokens||[];
          Object.assign(target,{...stronger,kind:'effect',trigger:target.trigger,condition:'none',text:'【'+target.trigger+'】'+stronger.text,bodyText:stronger.text,price:target.price+price});
          card.spent+=price;card.budget+=price;
          ...
          card.evolutionFocusBonus={trigger:target.trigger,raw:delta,price,naked,allowance,replaced:true};
        }
      }
    }
    ...
    if(cost>=2&&cost<=3&&!card.signature&&!card.sacrificeDesign){
      ...
      const setupOnly=early.length&&early.every(a=>['入场曲','谢幕曲'].includes(a.trigger)&&a.condition==='none'&&a.ids.every(id=>['earth','grave'].includes(id)));
      // Occasionally spend unused allowance on a setup carrier's body. This
      // does not grant stats or keywords, nor affect draws/ramp/early summons.
      const room=Math.floor(budget-card.spent-printedFloor);
      if(setupOnly&&room>=1&&rng(hash(seed+'|setup-body-allocation'))()<.45){
        printedFloor++;card.bodyAllowance=printedFloor;card.setupBodyAllocation={spent:1,resourceIds:early.flatMap(a=>a.ids)};
      }
    }
    ...
    if(cost===1){
      ...
      const summons=card.abilities.filter(a=>['入场曲','谢幕曲'].includes(a.trigger)&&a.condition==='none'&&(freeHandBoardValue(a)>0||a.ids.some(id=>['tokenSummon','fusionArtifactSummon','crystalHandSummon','experimentSummon'].includes(id))));
      if(summons.length){
    ...
    if(cost>=5&&used.has('疾驰')&&card.attack>0){
      ...
      if(assessment.value>stormLimit+1e-8||card.attack*assessment.attacks<Math.max(2,Math.floor(cost*.4))){
        // The effects alone leave no room for Storm. Remove it and restore
    ...
    if(card.stormRejected&&!options.skipStorm){
      // A rejected Storm plan has already reserved body and effect budget.
      // Rebuild without that plan rather than retain an underfunded shell or
      // append a fixed Rush/Ward/draw compensation.
      const rebuilt=generateCard(input,alternateConfig,{...options,skipStorm:true},skipProgression);
      rebuilt.stormRejected=true;rebuilt.stormRebuilt=true;return rebuilt;
    }
    ...
    if(cost>=6){
      ...
      if(card.corePower>=6&&!card.abilities.some(a=>a.major))card.abilities.reduce((a,b)=>a.raw>=b.raw?a:b).major=true;
    }
    ...
    const resurrection=card.abilities.find(a=>a.ids.includes('resurrectSelf'));
    if(resurrection){
      const spec=resurrection.resurrection;
      const retained=card.abilities.filter(a=>a!==resurrection&&a.kind!=='alternate'&&!['入场曲','进化时','超进化时','爆能强化','在手牌中发动','魔力增幅时','本卡牌被舍弃时'].includes(a.trigger)).reduce((n,a)=>n+a.price,0);
      const raw=((card.attack+card.health)*.8+retained*.7+2)*spec.repeats;
      const price=Math.max(3,raw-spec.credit);
      if(price<=card.budget-card.spent+resurrection.price-card.attack-card.health){
        card.spent+=price-resurrection.price;resurrection.raw=raw;resurrection.price=price;
        spec.body=(card.attack+card.health)*.8;spec.retained=retained;spec.finalValuation=true;
      }else{
        // The repeat engine consumed the core's reserved budget. If later
        // retained effects make it unaffordable, rebuild without that engine;
        // deleting it alone would leave a high-cost shell with trivial effects.
        return generateCard(input,alternateConfig,{...options,skipResurrection:true},skipProgression);
      }
    }
    ...
    const protectedStart=['潜行','威慑','屏障','damageCap','abilityDestructionImmune'].some(id=>used.has(id));
    for(const a of card.abilities.filter(a=>a.kind==='cycle')){
      const trigger=protectedStart?'自己的回合开始时':'自己的回合结束时';
      a.text=a.text.replace(/^自己的回合(?:开始|结束)时/,trigger);a.bodyText=a.text;a.trigger=trigger;
    }
