...
for(let i=0;i<12000;i++){
 ...
 if(!c.simpleDesign)continue;
 simple++;example||=c.name;
 assert.equal(c.type,'follower');assert.equal(c.rarity,0);assert(c.cost>=6);
 const active=c.abilities.filter(a=>a.kind!=='keyword');
 assert(active.length>=1&&active.length<=3,c.name);
 ...
 assert(c.corePower>=8&&active.some(a=>a.major),c.name);
 assert(c.attack+c.health+c.spent<=c.budget+.01,c.name);
}
assert(simple/highBronze>.6&&simple/highBronze<.8);
